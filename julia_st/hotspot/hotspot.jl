...
function single_iteration(result::Matrix{Float64}, temp::Matrix{Float64}, power::Matrix{Float64}, cap::Float64, rx::Float64, ry::Float64, rz::Float64, step::Float64)
    row = size(temp,1)
    col = size(temp,2)
    for r in 1:row
        for c in 1:col
...
#= Transient solver driver routine: simply converts the heat
 ...
 =#
function compute_tran_temp(result::Matrix{Float64}, num_iterations::Int32, temp::Matrix{Float64},
...
function main(args)
    ...
    result::Matrix{Float64} = zeros(Float64,grid_rows,grid_cols)
