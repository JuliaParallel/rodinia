...
#define AVIF_COPYRIGHTED 0x00020000
    ...
    // ThOe ->0
    //   OUTLONG(10000000);           /* MaxBytesPerSec, I hope this will never
    //   be used */
    OUTLONG(0);
    ...
    for (j = 0; j < AVI->anum; ++j) {
        ...
        /* Finish stream list, i.e. put number of bytes in the list to proper
         * pos */
...
int avi_parse_input_file(avi_t *AVI, int getIndex) {
    ...
    for (i = 0; i < AVI->n_idx; i++) {
        ...
        for (j = 0; j < AVI->anum; ++j)
            if (strncasecmp((char *)AVI->idx[i], AVI->track[j].audio_tag, 4) ==
                0)
                nai[j]++;
    }
