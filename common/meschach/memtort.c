    ...
    if (f != NULL) {
        if (mem_info_is_on()) {
            mem_bytes_list(TYPE_FOO_1,
                           sizeof(FOO_1) + f->dim * sizeof(Real[10]), 0,
                           FOO_LIST);
            mem_numvar_list(TYPE_FOO_1, -1, FOO_LIST);
        }
...
int foo_2_free(f) FOO_2 *f;
{
    if (f != NULL) {
        if (mem_info_is_on()) {
            mem_bytes_list(TYPE_FOO_2, sizeof(FOO_2) + f->dim * sizeof(Real[2]),
                           0, FOO_LIST);
            mem_numvar_list(TYPE_FOO_2, -1, FOO_LIST);
        }
...
char *argv[];
{
    ...
    mem_stat_mark(5);
    mem_stat_reg_list((void **)&foo_1, TYPE_FOO_1, FOO_LIST, __FILE__,
                      __LINE__);
    mem_stat_reg_list((void **)&foo_2, TYPE_FOO_2, FOO_LIST, __FILE__,
                      __LINE__);
