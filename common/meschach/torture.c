...
char *argv[];
{
    ...
    x = get_row(A, 3, x);
    y = get_col(B, 3, y);
    if (fabs(in_prod(x, y) - 1.0) >=
        MACHEPS * m_norm_inf(A) * m_norm_inf(B) * 5)
    ...
    for (i = 0; i < C->m; i++)
        for (j = 0; j < C->n; j++)
            m_set_val(C, i, j,
                      m_entry(C, i, j) + s1 * v_entry(w, i) * v_entry(w, j));
    LDLfactor(C);
    LDLupdate(A, w, s1);
...
#define LDIAG 2
    ...
    for (i = 0; i < B->m; i++)
        for (j = 0; j < B->n; j++)
            m_set_val(B, i, j,
                      m_entry(B, i, j) + v_entry(w, i) * v_entry(v, j));
    m_sub(B, C, C);
    ...
    m_sub(A, D, D);
    if (m_norm1(D) >=
        MACHEPS * m_norm1(Q) * m_norm_inf(Q) * v_norm_inf(u) * 3) {
        errmesg("symmeig()");
