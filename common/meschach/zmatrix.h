...
#define zm_set_val(A, i, j, val)                                               \
    (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] = (val)                            \
                        : (error(E_BOUNDS, "zm_set_val"), zmake(0.0, 0.0)))
...
#define zm_get_val(A, i, j)                                                    \
    (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)]                                    \
                        : (error(E_BOUNDS, "zm_get_val"), zmake(0.0, 0.0)))
#define zv_set_val(x, i, val)                                                  \
    (v_chk_idx(x, i) ? (x)->ve[(i)] = (val)                                    \
                     : (error(E_BOUNDS, "zv_set_val"), zmake(0.0, 0.0)))
#define zv_add_val(x, i, val)                                                  \
    (v_chk_idx(x, i) ? (x)->ve[(i)] = zadd((x)->ve[(i)], (val))                \
                     : (error(E_BOUNDS, "zv_set_val"), zmake(0.0, 0.0)))
#define zv_sub_val(x, i, val)                                                  \
    (v_chk_idx(x, i) ? (x)->ve[(i)] = zsub((x)->ve[(i)], (val))                \
                     : (error(E_BOUNDS, "zv_set_val"), zmake(0.0, 0.0)))
#define zv_get_val(x, i)                                                       \
    (v_chk_idx(x, i) ? (x)->ve[(i)]                                            \
...
extern complex _zin_prod(const ZVEC *x, const ZVEC *y, unsigned int i0,
                         unsigned int flag);
...
extern ZVEC *zv_mltadd(const ZVEC *v1, const ZVEC *v2, complex scale,
                       ZVEC *out);
...
extern ZVEC *_zv_map(complex (*f)(void *, complex), void *params, const ZVEC *x,
                     ZVEC *out);
#endif
...
extern complex __zip__(const complex zp1[], const complex zp2[], int len,
                       int flag);
extern void __zmltadd__(complex zp1[], const complex zp2[], complex s, int len,
                        int flag);
extern void __zmlt__(const complex zp[], complex s, complex out[], int len);
extern void __zadd__(const complex zp1[], const complex zp2[], complex out[],
                     int len);
extern void __zsub__(const complex zp1[], const complex zp2[], complex out[],
                     int len);
