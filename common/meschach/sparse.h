...
#define sprow_idx2(r, c, hint)                                                 \
    (((hint) >= 0 && (hint) < (r)->len && (r)->elt[hint].col == (c))           \
         ? (hint)                                                              \
         : sprow_idx((r), (c)))
...
#ifndef ANSI_C
extern SPMAT *sp_get(), *sp_copy(), *sp_copy2(), *sp_zero(), *sp_resize(),
    *sp_compact();
...
SPROW *sprow_get(int), *sprow_xpd(SPROW *r, int n, int type),
    ...
    *sprow_mltadd(const SPROW *r1, const SPROW *r2, double alpha, int j0,
                  SPROW *r_out, int type);
SPROW *sprow_add(const SPROW *r1, const SPROW *r2, int j0, SPROW *r_out,
                 int type),
    *sprow_sub(const SPROW *r1, const SPROW *r2, int j0, SPROW *r_out,
               int type),
...
#define fixindex(idx)                                                          \
    ((idx) == -1 ? (error(E_BOUNDS, "fixindex"), 0) : (idx) < 0 ? -((idx) + 2) \
                                                                : (idx))
