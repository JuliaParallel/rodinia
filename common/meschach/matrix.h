...
typedef struct { unsigned int size, max_size, *pe; } PERM;
...
#define RENEW(var, num, type)                                                  \
    ((var) =                                                                   \
         (type *)((var) ? realloc((char *)(var), (size_t)(num) * sizeof(type)) \
...
#define m_set_val(A, i, j, val)                                                \
    (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] = (val)                            \
                        : (error(E_BOUNDS, "m_set_val"), 0.0))
#define m_add_val(A, i, j, val)                                                \
    (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] += (val)                           \
                        : (error(E_BOUNDS, "m_add_val"), 0.0))
#define m_sub_val(A, i, j, val)                                                \
    (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)] -= (val)                           \
                        : (error(E_BOUNDS, "m_sub_val"), 0.0))
#define m_get_val(A, i, j)                                                     \
    (m_chk_idx(A, i, j) ? (A)->me[(i)][(j)]                                    \
                        : (error(E_BOUNDS, "m_get_val"), 0.0))
#define v_set_val(x, i, val)                                                   \
    (v_chk_idx(x, i) ? (x)->ve[(i)] = (val)                                    \
                     : (error(E_BOUNDS, "v_set_val"), 0.0))
#define v_add_val(x, i, val)                                                   \
    (v_chk_idx(x, i) ? (x)->ve[(i)] += (val)                                   \
                     : (error(E_BOUNDS, "v_set_val"), 0.0))
#define v_sub_val(x, i, val)                                                   \
    (v_chk_idx(x, i) ? (x)->ve[(i)] -= (val)                                   \
                     : (error(E_BOUNDS, "v_set_val"), 0.0))
#define v_get_val(x, i)                                                        \
    (v_chk_idx(x, i) ? (x)->ve[(i)] : (error(E_BOUNDS, "v_get_val"), 0.0))
#define bd_set_val(A, i, j, val)                                               \
    (bd_chk_idx(A, i, j) ? (A)->mat->me[(A)->lb + (j) - (i)][(j)] = (val)      \
                         : (error(E_BOUNDS, "bd_set_val"), 0.0))
#define bd_add_val(A, i, j, val)                                               \
    (bd_chk_idx(A, i, j) ? (A)->mat->me[(A)->lb + (j) - (i)][(j)] += (val)     \
                         : (error(E_BOUNDS, "bd_set_val"), 0.0))
#define bd_get_val(A, i, j)                                                    \
    (bd_chk_idx(A, i, j) ? (A)->mat->me[(A)->lb + (j) - (i)][(j)]              \
...
#ifndef ANSI_C
extern VEC *sv_mlt(), *mv_mlt(), *vm_mlt(), *v_add(), *v_sub(), *px_vec(),
    *pxinv_vec(), *v_mltadd(), *v_map(), *_v_map(), *v_lincomb(), *v_linlist();
...
extern VEC *sv_mlt(double s, const VEC *x, VEC *out), /* out <- s.x */
    ...
    *v_mltadd(const VEC *x, const VEC *y, double s,
              VEC *out), /* out <- x + s.y */
...
extern double _v_norm1(), _v_norm2(), _v_norm_inf(), m_norm1(), m_norm_inf(),
    m_norm_frob();
...
extern MAT *px_rows(), *px_cols(), *swap_rows(), *swap_cols(), *_set_row(),
    *_set_col();
extern VEC *get_row(), *get_col(), *sub_vec(), *mv_mltadd(), *vm_mltadd(),
    *bdv_mltadd();
...
extern PERM *px_mlt(const PERM *px1, const PERM *px2,
                    PERM *out),         /* out <- px1.px2 */
