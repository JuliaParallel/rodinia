...
#endif
{
    ...
    while (idx1 < r1->len) {
        ...
        if (!done_list->ive[e1->col]) {
            ...
            tracecatch(
                bkp_swap_elt(A, i1, e1->col, idx1, e1->col, i2, tmp_idx2),
                "bkp_interchange");
    ...
    while (idx1 < r1->len) {
        ...
        if (!done_list->ive[e1->col]) {
            ...
            tracecatch(
                bkp_swap_elt(A, i1, e1->col, idx1, i2, e1->col, tmp_idx2),
                "bkp_interchange");
            done_list->ive[e1->col] = TRUE;
        }
        idx1++;
    ...
    while (idx2 < r2->len) {
        ...
        if (!done_list->ive[e2->col]) {
            ...
            tracecatch(
                bkp_swap_elt(A, i2, e2->col, idx2, i1, e2->col, tmp_idx1),
                "bkp_interchange");
...
#endif
{
                ...
                for (s_idx2 = s_idx; s_idx2 < scan_row->dim; s_idx2++) {
                    ...
                    if (scan_row->ive[s_idx2] ==
                        ...
                        if (row->len >= row->maxlen) {
                            tracecatch(
                                sprow_xpd(row, 2 * row->maxlen + 1, TYPE_SPMAT),
                                "spBKPfactor");
                        }
            ...
            for (s_idx = 0; s_idx < scan_row->dim; s_idx++) {
                ...
                if (idx >= 0) {
                    ...
                    if (r_piv->len >= r_piv->maxlen) {
                        tracecatch(
                            sprow_xpd(r_piv, 2 * r_piv->maxlen + 1, TYPE_SPMAT),
                            "spBKPfactor");
                    }
    ...
    for (i = 0; i < A->m; i++)
        qsort(A->row[i].elt, A->row[i].len, sizeof(row_elt),
              (int (*)())col_cmp);
