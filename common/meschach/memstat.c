    ...
    if (mem_stat_var[k].var == var) {
        ...
        if (mem_stat_var[j].var == NULL)
            ...
            if (mem_stat_var[j].var == NULL)
                ...
                fprintf(stderr, "\n WARNING !!! static memory: mem_stat_var is "
                                "too small\n");
                fprintf(
                    stderr,
                ...
                if (!isatty(fileno(stdout))) {
                    fprintf(stdout, "\n WARNING !!! static memory: "
                                    "mem_stat_var is too small\n");
                    fprintf(stdout, " Increase MEM_HASHSIZE in file: %s "
                                    "(currently = %d)\n\n",
                            MEM_HASHSIZE_FILE, MEM_HASHSIZE);
                }
...
#ifdef DEBUG
                printf("# Freeing variable(s) registered in file \"%s\", line "
                       "%d\n",
...
#endif
{
    ...
    for (i = 0; i < mem_hash_idx_end; i++) {
        ...
        if (j == 0)
            ...
            j--;
            fprintf(
                fp, " %d.  var = 0x%p, type = %s, mark = %d\n", k,
                mem_stat_var[j].var,
                mem_stat_var[j].type < mem_connect[list].ntypes &&
                        mem_connect[list].free_funcs[mem_stat_var[j].type] !=
                            NULL
