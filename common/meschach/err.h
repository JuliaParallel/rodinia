...
extern int ev_err(const char *, int, int, const char *,
                  int);                /* main error handler */
...
/* warning(WARN_TYPE,"myfunc") raises warning type WARN_TYPE for
   function my_func() */
#define warning(err_num, fn_name)                                              \
...
#define catch (errnum, ok_part, err_part){                                     \
    ...
    _old_flag = set_err_flag(EF_SILENT); MEM_COPY(                             \
        restart, _save, sizeof(jmp_buf));                                      \
        if ((_err_num = setjmp(restart)) ==                                    \
             0){ok_part; set_err_flag(_old_flag); MEM_COPY(                    \
                _save, restart,                                                \
                sizeof(jmp_buf)); } else if (_err_num == errnum){              \
                                                 set_err_flag(_old_flag);      \
                                                 MEM_COPY(_save, restart,      \
                                                          sizeof(jmp_buf));    \
                                                          err_part; } else {   \
                                                     set_err_flag(_old_flag);  \
                                                     MEM_COPY(                 \
