...
#include <fcntl.h>
#include <stdlib.h>
#include [-"backprop1d.h"-] {+"backprop.h"+}
#include <math.h>
...
void bpnn_randomize_weights(w, m, n) float [-*w;-] {+**w;+}
int m, n;
{
    int i, j;
...
void bpnn_zero_weights(w, m, n) float [-*w;-] {+**w;+}
int m, n;
{
    int i, j;
...
BPNN *bpnn_internal_create(n_in, n_hidden, n_out) int n_in, n_hidden, n_out;
{
    ...
    newnet->input_weights = [-alloc_1d_dbl((n_in-] {+alloc_2d_dbl(n_in+} + [-1)*(n_hidden-] {+1, n_hidden+} + [-1));-] {+1);+}
    newnet->hidden_weights = [-alloc_1d_dbl((n_hidden-] {+alloc_2d_dbl(n_hidden+} + [-1)*(n_out-] {+1, n_out+} + [-1));-] {+1);+}

    newnet->input_prev_weights = [-alloc_1d_dbl((n_in-] {+alloc_2d_dbl(n_in+} + [-1)*(n_hidden-] {+1, n_hidden+} + [-1));-] {+1);+}
    newnet->hidden_prev_weights = [-alloc_1d_dbl((n_hidden-] {+alloc_2d_dbl(n_hidden+} + [-1)*(-] {+1,+} n_out + [-1));-] {+1);+}
...
void bpnn_layerforward(l1, l2, conn, n1, n2) float *l1, *l2, [-*conn;-] {+**conn;+}
int n1, n2;
{
    clock_t start, end;
...
#pragma omp parallel for shared(conn, n1, n2, l1) private(k, j) reduction(     \
    ...
    for (j = 1; j <= n2; j++) {
        ...
        float sum = 0.0;
        for (k = 0; k <= n1; k++) {
            sum += [-conn[k*(n2+1)+j]-] {+conn[k][j]+} * l1[k];
        }
        l2[j] = squash(sum);
    }
#ifdef OMP_OFFLOAD
...
int nh, no;
{
    ...
    for (j = 1; j <= nh; j++) {
        ...
        sum = 0.0;
        for (k = 1; k <= no; k++) {
            sum += delta_o[k] * [-who[j*(no+1)+k];-] {+who[j][k];+}
        }
...
#pragma omp target exit data map(always, from: [-oldw[:(nly+1)*(ndelta+1)], w[:(nly+1)*(ndelta+1)],-] {+w[:nly+1][:ndelta+1])
#else
#pragma omp target exit data map(always, from:+} delta[:ndelta+1], ly[:nly+1])
[-//-]
    for (int k = 0; k <= nly; k++) {
...
BPNN *bpnn_read(filename) char *filename;
{
    ...
    for (i = 0; i <= n1; i++) {
        for (j = 0; j <= n2; j++) {
            [-fastcopy(&(new->input_weights[i*(n2+1)+j]),-]
    ...
    for (i = 0; i <= n2; i++) {
        for (j = 0; j <= n3; j++) {
            [-fastcopy(&(new->hidden_weights[i*(n2+1)+j]),-]
