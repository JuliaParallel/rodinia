...
#pragma omp parallel for default(shared) schedule(static)
    for (int i = 0; i < nelr; i++) {
        ...
        // dt = double(0.5) * std::sqrt(areas[i]) /  (||v|| + c).... but when we
...
#pragma omp parallel for default(shared) schedule(static)
    for (int i = 0; i < nelr; i++) {
        ...
        compute_flux_contribution(density_i, momentum_i, density_energy_i,
                                  pressure_i, velocity_i, fc_i_momentum_x,
                                  fc_i_momentum_y, fc_i_momentum_z,
                                  fc_i_density_energy);
    ...
    for (int i = 0; i < nelr; i++) {
        ...
        for (j = 0; j < NNB; j++) {
            ...
            } else if (nb == -2) // a far field boundary
            {
                ...
                flux_i_momentum.x +=
                    factor * (ff_fc_momentum_x.x + fc_i_momentum_x.x);
                flux_i_momentum.y +=
                    factor * (ff_fc_momentum_y.x + fc_i_momentum_y.x);
                flux_i_momentum.z +=
                    factor * (ff_fc_momentum_z.x + fc_i_momentum_z.x);
                ...
                flux_i_momentum.x +=
                    factor * (ff_fc_momentum_x.y + fc_i_momentum_x.y);
                flux_i_momentum.y +=
                    factor * (ff_fc_momentum_y.y + fc_i_momentum_y.y);
                flux_i_momentum.z +=
                    factor * (ff_fc_momentum_z.y + fc_i_momentum_z.y);
                ...
                flux_i_momentum.x +=
                    factor * (ff_fc_momentum_x.z + fc_i_momentum_x.z);
                flux_i_momentum.y +=
                    factor * (ff_fc_momentum_y.z + fc_i_momentum_y.z);
                flux_i_momentum.z +=
                    factor * (ff_fc_momentum_z.z + fc_i_momentum_z.z);
            }
        }
...
#pragma omp parallel for default(shared) schedule(static)
    for (int i = 0; i < nelr; i++) {
        ...
        variables[NVAR * i + VAR_DENSITY] =
            old_variables[NVAR * i + VAR_DENSITY] +
    ...
    double *normals;
    {
        ...
        for (int i = 0; i < nel; i++) {
            ...
            for (int j = 0; j < NNB; j++) {
                ...
                elements_surrounding_elements[i * NNB + j]--; // it's coming in
                                                              // with Fortran
                                                              // numbering
