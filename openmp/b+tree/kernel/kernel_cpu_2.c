        ...
        for (i = 0; i < maxheight; i++) {
            ...
            for (thid = 0; thid < threadsPerBlock; thid++) {
                ...
                if ((knodes[currKnode[bid]].keys[thid] <= start[bid]) &&
                    ...
                    // "offset[bid]" calculated below that later addresses part
                    // of knodes goes outside of its bounds cause segmentation
                    // fault
                ...
                if ((knodes[lastKnode[bid]].keys[thid] <= end[bid]) &&
                    ...
                    // this conditional statement is inserted to avoid crush due
                    // to but in original code
                    // "offset_2[bid]" calculated below that later addresses
                    // part of knodes goes outside of its bounds cause
                    // segmentation fault
                    // more specifically, values saved into knodes->indices in
                    // the main function are out of bounds of knodes that they
                    // address
