...
// cells
__kernel void IMGVF_kernel(__global FP_TYPE *IMGVF_array,
                           __global FP_TYPE *I_array, __constant int *I_offsets,
                           int __constant *m_array, __constant int *n_array,
                           FP_TYPE vx, FP_TYPE vy, FP_TYPE e,
                           int max_iterations, FP_TYPE cutoff) {
    ...
    while ((!cell_converged) && (iterations < max_iterations)) {
        ...
        for (thread_block = 0; thread_block < tb_count; thread_block++) {
            ...
            if (i < m) {
                ...
                // Keep track of the total change of this thread's matrix
                // elements
