...
float *GICOV_OpenCL(int grad_m, int grad_n, float *host_grad_x,
                    float *host_grad_y) {
    ...
    device_grad_x =
        clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                        &image_format, grad_m, grad_n, 0, host_grad_x, &error);
    check_error(error, __FILE__, __LINE__);
    device_grad_y =
        clCreateImage2D(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                        &image_format, grad_m, grad_n, 0, host_grad_y, &error);
    check_error(error, __FILE__, __LINE__);
#else
    // Create buffers
    device_grad_x =
        clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                       grad_mem_size, host_grad_x, &error);
    check_error(error, __FILE__, __LINE__);
    device_grad_y =
        clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                       grad_mem_size, host_grad_y, &error);
    check_error(error, __FILE__, __LINE__);
#endif

    // Allocate & initialize device memory for result
    // (some elements are not assigned values in the kernel)
    // Since there is no OpenCL version of cudaMemset, we first allocate and
    // initialize
    ...
    device_gicov = clCreateBuffer(
        context,
        CL_MEM_READ_WRITE | /*CL_MEM_COPY_HOST_PTR*/ CL_MEM_USE_HOST_PTR,
        grad_mem_size, host_gicov, &error);
    check_error(error, __FILE__, __LINE__);
    ...
    if (global_work_size % work_group_size > 0)
        global_work_size =
            (global_work_size / work_group_size + 1) * work_group_size;

    printf("Find: local_work_size = %d, global_work_size = %d \n",
           work_group_size, global_work_size);

    // Execute the GICOV kernel
    error = clEnqueueNDRangeKernel(command_queue, GICOV_kernel, 1, NULL,
                                   &global_work_size, &work_group_size, 0, NULL,
                                   NULL);
    check_error(error, __FILE__, __LINE__);
    ...
    host_gicov = (cl_float *)clEnqueueMapBuffer(
        command_queue, device_gicov, CL_TRUE, CL_MAP_READ, 0, grad_mem_size, 0,
        NULL, NULL, &error);
    check_error(error, __FILE__, __LINE__);
...
float *dilate_OpenCL(int max_gicov_m, int max_gicov_n, int strel_m,
                     int strel_n) {
    ...
    cl_mem device_img_dilated = clCreateBuffer(
        context, CL_MEM_WRITE_ONLY, max_gicov_mem_size, NULL, &error);
    check_error(error, __FILE__, __LINE__);
    ...
    image_format.image_channel_data_type = CL_FLOAT;
    // Create the image
    cl_mem device_gicov_image =
        clCreateImage2D(context, CL_MEM_READ_ONLY, &image_format, max_gicov_m,
                        max_gicov_n, 0, NULL, &error);
    check_error(error, __FILE__, __LINE__);
    // Copy the GICOV data to the image
    size_t offset[3] = {0, 0, 0};
    size_t region[3] = {max_gicov_m, max_gicov_n, 1};
    error = clEnqueueCopyBufferToImage(command_queue, device_gicov,
                                       device_gicov_image, 0, offset, region, 0,
                                       NULL, NULL);
    check_error(error, __FILE__, __LINE__);
#endif
...
#ifdef USE_IMAGE
    clSetKernelArg(dilate_kernel, 5, sizeof(cl_mem),
                   (void *)&device_gicov_image);
#else
    clSetKernelArg(dilate_kernel, 5, sizeof(cl_mem), (void *)&device_gicov);
#endif
    clSetKernelArg(dilate_kernel, 6, sizeof(cl_mem),
                   (void *)&device_img_dilated);

    // Execute the dilation kernel
    error = clEnqueueNDRangeKernel(command_queue, dilate_kernel, 1, NULL,
    ...
    // Copy the result to the host
    // float *host_img_dilated = (cl_float *) clEnqueueMapBuffer(command_queue,
    // device_img_dilated, CL_TRUE, CL_MAP_READ, 0, max_gicov_mem_size, 0, NULL,
    // NULL, &error);
    float *host_img_dilated = (float *)malloc(max_gicov_mem_size);
    error = clEnqueueReadBuffer(command_queue, device_img_dilated, CL_TRUE, 0,
                                max_gicov_mem_size, host_img_dilated, 0, NULL,
                                NULL);
    check_error(error, __FILE__, __LINE__);
...
void select_device() {
    ...
    // Iterate through all available platforms, choosing the first one that has
    // a GPU
    int i;
    for (i = 0; i < num_platforms; i++) {
        ...
        context = clCreateContextFromType(ctxprop, CL_DEVICE_TYPE_GPU, NULL,
                                          NULL, &error);
        ...
        error = clGetContextInfo(context, CL_CONTEXT_DEVICES, size, device_list,
                                 NULL);
        check_error(error, __FILE__, __LINE__);
        ...
        char cBuffer[1024];
        clGetDeviceInfo(device_list[0], CL_DEVICE_NAME, sizeof(cBuffer),
                        &cBuffer, NULL);
        printf("Running on: %s\n", cBuffer);
...
void transfer_constants(float *host_sin_angle, float *host_cos_angle,
    ...
    c_sin_angle =
        clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                       angle_mem_size, host_sin_angle, &error);
    check_error(error, __FILE__, __LINE__);
    c_cos_angle =
        clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                       angle_mem_size, host_cos_angle, &error);
    check_error(error, __FILE__, __LINE__);
    c_tX = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                          t_mem_size, host_tX, &error);
    check_error(error, __FILE__, __LINE__);
    c_tY = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                          t_mem_size, host_tY, &error);
    check_error(error, __FILE__, __LINE__);
    c_strel = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                             strel_mem_size, host_strel, &error);
    check_error(error, __FILE__, __LINE__);
}
