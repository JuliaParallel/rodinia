...
MAT *get_frame(avi_t *cell_file, int frame_num, int cropped, int scaled) {
    ...
    if (cropped) {
        ...
        image_chopped = chop_flip_image(image_buf, height, width, TOP, BOTTOM,
                                        0, width - 1, scaled);
    } else {
        // Just flip the image
        image_chopped = chop_flip_image(image_buf, height, width, 0, height - 1,
                                        0, width - 1, scaled);
    }
    ...
    if (scaled) {
        ...
        for (i = 0; i <= (bottom - top); i++)
            for (j = 0; j <= (right - left); j++)
                // m_set_val(result, i, j, (double) image[((height - (i + top))
                // * width) + (j + left)] * scale);
                m_set_val(result, i, j,
                          (double)image[((height - 1 - (i + top)) * width) +
                                        (j + left)] *
                              scale);
    } else {
        for (i = 0; i <= (bottom - top); i++)
            for (j = 0; j <= (right - left); j++)
                ...
                m_set_val(
                    result, i, j,
                    (double)
                        image[((height - 1 - (i + top)) * width) + (j + left)]);
    }
...
void compute_constants() {
    ...
    transfer_constants(host_sin_angle, host_cos_angle, host_tX, host_tY,
                       strel_m, strel_n, host_strel);

    // Free the memory (only need to free strel since the rest are declared
    // statically)
    free(host_strel);
}
...
MAT *dilate(MAT *img_in) {
    ...
    float *host_img_dilated =
        dilate_OpenCL(max_gicov_m, max_gicov_n, strel_m, strel_n);
...
MAT *TMatrix(unsigned int N, unsigned int M) {
    MAT *B = NULL, *LB = NULL, *B_TEMP = NULL, *B_TEMP_INV = NULL,
        *B_RET = NULL;
...
VEC *getsampling(MAT *m, int ns) {
    ...
    for (i = 0; i < N; i++) {
        for (j = 0; j < M; j++) {
            ...
            b = m_get_val(m, 0, bindex[i]) *
                (3.0 * s * s * s - 6.0 * s * s + 4.0);
...
MAT *linear_interp2(MAT *m, VEC *X, VEC *Y) {
    ...
    for (i = 0; i < X->dim; i++) {
        ...
        // printf("xc: %f \t yc: %f \t i: %d \t l: %d \t k: %d \t a: %f \t b:
...
void splineenergyform01(MAT *Cx, MAT *Cy, MAT *Ix, MAT *Iy, int ns,
    ...
    VEC *X, *Y, *Xs, *Ys, *Nx, *Ny, *X1, *Y1, *X2, *Y2, *XY, *XX, *YY, *dCx,
        *dCy, *Ix1, *Ix2, *Iy1, *Iy2;
    ...
    for (i = 0; i < Cx->m; i++) {
        for (j = 0; j < ns; j++) {
            ...
            double A1, A2, A3, A4, B1, B2, B3, B4, D, D_3, Tx1, Tx2, Tx3, Tx4,
                Ty1, Ty2, Ty3, Ty4;
            int k;
