...
#ifdef USE_IMAGE
__kernel void GICOV_kernel(int grad_m, image2d_t grad_x, image2d_t grad_y,
                           __constant float *c_sin_angle,
                           __constant float *c_cos_angle, __constant int *c_tX,
                           __constant int *c_tY, __global float *gicov) {
#else
__kernel void GICOV_kernel(int grad_m, __global float *grad_x,
...
// Compute the combined gradient value at the current sample point
            ...
            int addr = x * grad_m + y;
            float p =
                grad_x[addr] * c_cos_angle[n] + grad_y[addr] * c_sin_angle[n];
#endif
...
#ifdef USE_IMAGE
__kernel void dilate_kernel(int img_m, int img_n, int strel_m, int strel_n,
                            __constant float *c_strel, image2d_t img,
                            __global float *dilated) {
#else
__kernel void dilate_kernel(int img_m, int img_n, int strel_m, int strel_n,
                            __constant float *c_strel, __global float *img,
                            __global float *dilated) {
#endif
        ...
        for (el_i = 0; el_i < strel_m; el_i++) {
            ...
            if ((y >= 0) && (y < img_m)) {
                ...
                for (el_j = 0; el_j < strel_n; el_j++) {
                    ...
                    // Make sure we have not gone off the edge of the matrix
                    //  and that the current structuring element value is not
                    //  zero
                    if ((x >= 0) && (x < img_n) &&
