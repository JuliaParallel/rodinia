...
// Host function that launches an OpenCL kernel to compute the MGVF matrices for
// the specified cells
void IMGVF_OpenCL(MAT **I, MAT **IMGVF, double vx, double vy, double e,
    ...
    error = clEnqueueNDRangeKernel(command_queue, IMGVF_kernel, 1, NULL,
...
void IMGVF_OpenCL_init(MAT **IE, int num_cells) {
    ...
    device_I_offsets =
        clCreateBuffer(context, CL_MEM_READ_ONLY, mem_size, NULL, &error);
    check_error(error, __FILE__, __LINE__);
    ...
    device_I_all =
        clCreateBuffer(context, CL_MEM_READ_ONLY, total_mem_size, NULL, &error);
    check_error(error, __FILE__, __LINE__);
    device_IMGVF_all = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                      total_mem_size, NULL, &error);
    check_error(error, __FILE__, __LINE__);
    ...
    error =
        clEnqueueWriteBuffer(command_queue, device_I_all, CL_TRUE, 0,
                             total_mem_size, (void *)host_I_all, 0, NULL, NULL);
    check_error(error, __FILE__, __LINE__);
    error =
        clEnqueueWriteBuffer(command_queue, device_IMGVF_all, CL_TRUE, 0,
                             total_mem_size, (void *)host_I_all, 0, NULL, NULL);
    check_error(error, __FILE__, __LINE__);

    // Copy offsets array to device
    error =
        clEnqueueWriteBuffer(command_queue, device_I_offsets, CL_TRUE, 0,
                             mem_size, (void *)host_I_offsets, 0, NULL, NULL);
    check_error(error, __FILE__, __LINE__);

    // Copy memory dimension arrays to device
    error = clEnqueueWriteBuffer(command_queue, device_m_array, CL_TRUE, 0,
                                 mem_size, (void *)host_m_array, 0, NULL, NULL);
    check_error(error, __FILE__, __LINE__);
    error = clEnqueueWriteBuffer(command_queue, device_n_array, CL_TRUE, 0,
                                 mem_size, (void *)host_n_array, 0, NULL, NULL);
    check_error(error, __FILE__, __LINE__);
}
...
void IMGVF_OpenCL_cleanup(MAT **IMGVF_out_array, int num_cells) {
    ...
    float *host_IMGVF_all = (cl_float *)clEnqueueMapBuffer(
        command_queue, device_IMGVF_all, CL_TRUE, CL_MAP_READ, 0,
        total_mem_size, 0, NULL, NULL, &error);
    check_error(error, __FILE__, __LINE__);
        ...
        for (i = 0; i < m; i++)
            for (j = 0; j < n; j++)
                m_set_val(IMGVF_out, i, j,
                          (double)host_IMGVF_all[offset + (i * n) + j]);

        offset += (m * n);
    }

    // Unmap results buffer
    error = clEnqueueUnmapMemObject(command_queue, device_IMGVF_all,
                                    (void *)host_IMGVF_all, 0, NULL, NULL);
    check_error(error, __FILE__, __LINE__);
