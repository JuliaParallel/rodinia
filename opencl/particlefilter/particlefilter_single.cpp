...
void ocl_print_float_array(cl_command_queue cmd_q, cl_mem array_GPU,
                           size_t size) {
...
/*
 * @brief sets up the OpenCL framework by detecting and initializing the
 * available device
...
static int initialize(int use_gpu) {
    ...
    if (err != CL_SUCCESS) {
        if (err == CL_INVALID_VALUE)
            printf("clGetPlatformIDs() returned invalid_value\n");
    ...
    if (!context) {
        if (CL_INVALID_PLATFORM == err)
            printf(
                "CL_INVALID_PLATFORM returned by clCreateContextFromType()\n");
            ...
            printf("CL_INVALID_DEVICE_TYPE returned by "
                   "clCreateContextFromType()\n");
        else if (CL_INVALID_OPERATION == err)
            printf(
                "CL_INVALID_OPERATION returned by clCreateContextFromType()\n");
        else if (CL_DEVICE_NOT_AVAILABLE == err)
            printf("CL_DEVICE_NOT_AVAILABLE returned by "
                   "clCreateContextFromType()\n");
        else if (CL_DEVICE_NOT_FOUND == err)
            printf(
                "CL_DEVICE_NOT_FOUND returned by clCreateContextFromType()\n");
        else if (CL_OUT_OF_RESOURCES == err)
            printf(
                "CL_OUT_OF_RESOURCES returned by clCreateContextFromType()\n");
 ...
 * @param seed The seed array
 * @param index The specific index of the seed to be advanced
 * @return a float representing random number generated using the Box-Muller
 * algorithm
...
void strelDisk(int *disk, int radius) {
    ...
    for (x = 0; x < diameter; x++) {
        for (y = 0; y < diameter; y++) {
            float distance = sqrt(pow((float)(x - radius + 1), 2) +
                                  pow((float)(y - radius + 1), 2));
            if (distance < radius)
                disk[x * diameter + y] = 1;
            else
                disk[x * diameter + y] = 0;
...
void dilate_matrix(unsigned char *matrix, int posX, int posY, int posZ,
    ...
    for (x = startX; x < endX; x++) {
        for (y = startY; y < endY; y++) {
            float distance =
                sqrt(pow((float)(x - posX), 2) + pow((float)(y - posY), 2));
            if (distance < error)
...
int particleFilter(unsigned char *I, int IszX, int IszY, int Nfr, int *seed,
    ...
    err = clBuildProgram(prog, 1, device_list, "-cl-fast-relaxed-math", NULL,
                         NULL);
    ...
    //     err = clGetContextInfo(context, CL_CONTEXT_DEVICES, sizeof
    ...
    if (err != CL_SUCCESS) {
        if (err == CL_INVALID_PROGRAM)
            printf("ERROR: clCreateKernel(likelihood_kernel) 0 => INVALID "
                   "PROGRAM %d\n",
                   err);
        if (err == CL_INVALID_PROGRAM_EXECUTABLE)
            printf("ERROR: clCreateKernel(likelihood_kernel) 0 => INVALID "
                   "PROGRAM EXECUTABLE %d\n",
                   err);
    ...
    kernel_normalize_weights =
        clCreateKernel(prog, s_normalize_weights_kernel, &err);
    if (err != CL_SUCCESS) {
    ...
    arrayX_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
        printf("ERROR: clCreateBuffer arrayX_GPU (size:%d) => %d\n", Nparticles,
               err);
        return -1;
    }
    arrayY_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    xj_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                            sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    yj_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                            sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    CDF_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                             sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    u_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                           sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    likelihood_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                    sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    weights_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                 sizeof(float) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    I_GPU =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       sizeof(unsigned char) * IszX * IszY * Nfr, NULL, &err);
    if (err != CL_SUCCESS) {
        printf("ERROR: clCreateBuffer I_GPU (size:%d) => %d\n",
               IszX * IszY * Nfr, err);
        return -1;
    }
    objxy_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                               2 * sizeof(int) * countOnes, NULL, &err);
    if (err != CL_SUCCESS) {
        printf("ERROR: clCreateBuffer objxy_GPU (size:%d) => %d\n", countOnes,
               err);
        return -1;
    }
    ind_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                             sizeof(int) * countOnes * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
        printf("ERROR: clCreateBuffer ind_GPU (size:%d) => %d\n",
               countOnes * Nparticles, err);
        return -1;
    }
    seed_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                              sizeof(int) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    partial_sums =
        clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                       sizeof(float) * Nparticles + 1, likelihood, &err);
    if (err != CL_SUCCESS) {
    ...
    // OpenCL memory copy
    err = clEnqueueWriteBuffer(cmd_queue, I_GPU, 1, 0,
                               sizeof(unsigned char) * IszX * IszY * Nfr, I, 0,
                               0, 0);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(cmd_queue, objxy_GPU, 1, 0,
    ...
    err = clEnqueueWriteBuffer(cmd_queue, weights_GPU, 1, 0,
                               sizeof(float) * Nparticles, weights, 0, 0, 0);
    if (err != CL_SUCCESS) {
        printf("ERROR: clEnqueueWriteBuffer weights_GPU (size:%d) => %d\n",
               Nparticles, err);
        return -1;
    }
    err = clEnqueueWriteBuffer(cmd_queue, xj_GPU, 1, 0,
                               sizeof(float) * Nparticles, xj, 0, 0, 0);
    if (err != CL_SUCCESS) {
        printf("ERROR: clEnqueueWriteBuffer arrayX_GPU (size:%d) => %d\n",
               Nparticles, err);
        return -1;
    }
    err = clEnqueueWriteBuffer(cmd_queue, yj_GPU, 1, 0,
                               sizeof(float) * Nparticles, yj, 0, 0, 0);
    if (err != CL_SUCCESS) {
        printf("ERROR: clEnqueueWriteBuffer arrayY_GPU (size:%d) => %d\n",
               Nparticles, err);
        return -1;
    }
    err = clEnqueueWriteBuffer(cmd_queue, seed_GPU, 1, 0,
    ...
    for (k = 1; k < Nfr; k++) {
        /****************** L I K E L I H O O D
         * ************************************/
        clSetKernelArg(kernel_likelihood, 0, sizeof(void *),
                       (void *)&arrayX_GPU);
        clSetKernelArg(kernel_likelihood, 1, sizeof(void *),
                       (void *)&arrayY_GPU);
        ...
        clSetKernelArg(kernel_likelihood, 6, sizeof(void *),
                       (void *)&objxy_GPU);
        clSetKernelArg(kernel_likelihood, 7, sizeof(void *),
                       (void *)&likelihood_GPU);
        clSetKernelArg(kernel_likelihood, 8, sizeof(void *), (void *)&I_GPU);
        clSetKernelArg(kernel_likelihood, 9, sizeof(void *), (void *)&u_GPU);
        clSetKernelArg(kernel_likelihood, 10, sizeof(void *),
                       (void *)&weights_GPU);
        clSetKernelArg(kernel_likelihood, 11, sizeof(cl_int),
                       (void *)&Nparticles);
        clSetKernelArg(kernel_likelihood, 12, sizeof(cl_int),
                       (void *)&countOnes);
        clSetKernelArg(kernel_likelihood, 13, sizeof(cl_int),
                       (void *)&max_size);
        ...
        clSetKernelArg(kernel_likelihood, 17, sizeof(void *),
                       (void *)&seed_GPU);
        clSetKernelArg(kernel_likelihood, 18, sizeof(void *),
                       (void *)&partial_sums);
        clSetKernelArg(kernel_likelihood, 19, threads_per_block * sizeof(float),
                       NULL);

        // KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_likelihood, 1, NULL,
                                     global_work, local_work, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
            printf(
                "ERROR: clEnqueueNDRangeKernel(kernel_likelihood)=>%d failed\n",
                err);
        ...
        // KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_sum, 1, NULL,
                                     global_work, local_work, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
        ...
        clSetKernelArg(kernel_normalize_weights, 0, sizeof(void *),
                       (void *)&weights_GPU);
        clSetKernelArg(kernel_normalize_weights, 1, sizeof(cl_int),
                       (void *)&Nparticles);
        clSetKernelArg(kernel_normalize_weights, 2, sizeof(void *),
                       (void *)&partial_sums); //*/
        clSetKernelArg(kernel_normalize_weights, 3, sizeof(void *),
                       (void *)&CDF_GPU);
        clSetKernelArg(kernel_normalize_weights, 4, sizeof(void *),
                       (void *)&u_GPU);
        clSetKernelArg(kernel_normalize_weights, 5, sizeof(void *),
                       (void *)&seed_GPU);

        // KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_normalize_weights, 1,
                                     NULL, global_work, local_work, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
            printf(
                "ERROR: clEnqueueNDRangeKernel(normalize_weights)=>%d failed\n",
                err);
            // check_error(err, __FILE__, __LINE__);
            return -1;
        }
        ...
        err = clEnqueueReadBuffer(cmd_queue, arrayX_GPU, 1, 0,
                                  sizeof(float) * Nparticles, arrayX, 0, 0, 0);
        err = clEnqueueReadBuffer(cmd_queue, arrayY_GPU, 1, 0,
                                  sizeof(float) * Nparticles, arrayY, 0, 0, 0);
        err = clEnqueueReadBuffer(cmd_queue, weights_GPU, 1, 0,
                                  sizeof(float) * Nparticles, weights, 0, 0, 0);
        ...
        for (x = 0; x < Nparticles; x++) {
            // if( 0.0000000 < arrayX[x]*weights[x]) printf("arrayX[%d]:%f,
            // arrayY[%d]:%f, weights[%d]:%0.10f\n",x,arrayX[x], x, arrayY[x],
            // x, weights[x]);
        ...
        printf("YE: %lf\n", ye);
        float distance =
            sqrt(pow((float)(xe - (int)roundFloat(IszY / 2.0)), 2) +
        ...
        clSetKernelArg(kernel_find_index, 0, sizeof(void *),
                       (void *)&arrayX_GPU);
        clSetKernelArg(kernel_find_index, 1, sizeof(void *),
                       (void *)&arrayY_GPU);
        ...
        clSetKernelArg(kernel_find_index, 6, sizeof(void *),
                       (void *)&weights_GPU);
        clSetKernelArg(kernel_find_index, 7, sizeof(cl_int),
                       (void *)&Nparticles);
        // KERNEL FUNCTION CALL
        err = clEnqueueNDRangeKernel(cmd_queue, kernel_find_index, 1, NULL,
                                     global_work, local_work, 0, 0, 0);
        clFinish(cmd_queue);
        if (err != CL_SUCCESS) {
