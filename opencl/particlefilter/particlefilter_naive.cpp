...
static int allocate(int Nparticles, int countOnes) {
    ...
    { // show warnings/errors
        ...
        err = clGetProgramBuildInfo(prog, device_id[0], CL_PROGRAM_BUILD_LOG,
 ...
 */
int particleFilter(int *I, int IszX, int IszY, int Nfr, int *seed,
                   int Nparticles) {
    int max_size = IszX * IszY * Nfr;
    ...
    for (k = 1; k < Nfr; k++) {
        ...
        // OpenCL memory allocation
        cl_int err =
            clEnqueueWriteBuffer(cmd_queue, arrayX_GPU, 1, 0,
        ...
        err =
            clEnqueueWriteBuffer(cmd_queue, arrayY_GPU, 1, 0,
        ...
        err = clEnqueueWriteBuffer(cmd_queue, CDF_GPU, 1, 0,
                                   sizeof(double) * Nparticles, CDF, 0, 0, 0);
        if (err != CL_SUCCESS) {
            printf("ERROR: clEnqueueWriteBuffer CDF_GPU (size:%d) => %d\n",
                   Nparticles, err);
            return -1;
        }
        err = clEnqueueWriteBuffer(cmd_queue, u_GPU, 1, 0,
                                   sizeof(double) * Nparticles, u, 0, 0, 0);
        if (err != CL_SUCCESS) {
            printf("ERROR: clEnqueueWriteBuffer u_GPU (size:%d) => %d\n",
                   Nparticles, err);
            return -1;
        }
        ...
        err = clEnqueueReadBuffer(cmd_queue, yj_GPU, 1, 0,
                                  sizeof(double) * Nparticles, yj, 0, 0, 0);
        if (err != CL_SUCCESS) {
            printf("ERROR: Memcopy Out\n");
            return -1;
        }
        err = clEnqueueReadBuffer(cmd_queue, xj_GPU, 1, 0,
        ...
        // FIXED: this used to be a memory leak where arrayX was assigned the
        // address of xj
        for (x = 0; x < Nparticles; x++) {
            arrayX[x] = xj[x];
            arrayY[x] = yj[x];
