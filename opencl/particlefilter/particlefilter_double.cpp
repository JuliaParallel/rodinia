...
int particleFilter(unsigned char *I, int IszX, int IszY, int Nfr, int *seed,
    ...
    arrayX_GPU = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                sizeof(double) * Nparticles, NULL, &err);
    if (err != CL_SUCCESS) {
    ...
    partial_sums =
        clCreateBuffer(context, CL_MEM_READ_WRITE | CL_MEM_COPY_HOST_PTR,
                       sizeof(double) * Nparticles + 1, likelihood, &err);
    if (err != CL_SUCCESS) {
    ...
    for (k = 1; k < Nfr; k++) {
        ...
        clSetKernelArg(kernel_likelihood, 18, sizeof(void *),
                       (void *)&partial_sums);
        clSetKernelArg(kernel_likelihood, 19,
                       threads_per_block * sizeof(double), NULL);
        ...
        err =
            clEnqueueReadBuffer(cmd_queue, weights_GPU, 1, 0,
                                sizeof(double) * Nparticles, weights, 0, 0, 0);
