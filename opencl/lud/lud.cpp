...
int main(int argc, char *argv[]) {
    ...
    cl_mem d_m;
    d_m = clCreateBuffer(context, CL_MEM_READ_WRITE,
    ...
    stopwatch_start(&sw);
    err = clEnqueueWriteBuffer(cmd_queue, d_m, 1, 0,
    ...
    for (i = 0; i < matrix_dim - BLOCK_SIZE; i += BLOCK_SIZE) {
        ...
        err = clEnqueueNDRangeKernel(cmd_queue, diagnal, 2, NULL, global_work1,
                                     local_work1, 0, 0, 0);
        if (err != CL_SUCCESS) {
        ...
        err = clEnqueueNDRangeKernel(cmd_queue, perimeter, 2, NULL,
        ...
        err = clEnqueueNDRangeKernel(cmd_queue, internal, 2, NULL, global_work3,
                                     local_work3, 0, 0, 0);
        if (err != CL_SUCCESS) {
    ...
    err = clEnqueueReadBuffer(cmd_queue, d_m, 1, 0,
                              matrix_dim * matrix_dim * sizeof(float), m, 0, 0,
                              0);
    if (err != CL_SUCCESS) {
