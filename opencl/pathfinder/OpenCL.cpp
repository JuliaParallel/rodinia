...
OpenCL::OpenCL(int displayOutput) { VERBOSE = displayOutput; }
...
size_t OpenCL::localSize() { return this->lwsize; }

cl_command_queue OpenCL::q() { return this->command_queue; }

void OpenCL::launch(string toLaunch) {
    // Launch the kernel (or at least enqueue it).
    ret = clEnqueueNDRangeKernel(command_queue, kernelArray[toLaunch], 1, NULL,
                                 &gwsize, &lwsize, 0, NULL, NULL);

    if (ret != CL_SUCCESS) {
        printf("\nError attempting to launch %s. Error in "
               "clCreateProgramWithSource with error code %i\n\n",
               toLaunch.c_str(), ret);
        exit(1);
    }
}

void OpenCL::gwSize(size_t theSize) { this->gwsize = theSize; }

cl_context OpenCL::ctxt() { return this->context; }
...
void OpenCL::createKernel(string kernelName) {
    ...
    clGetKernelWorkGroupInfo(kernelArray[kernelName], device_id[0],
                             ...
                             NULL);
    if (lwsize == 0) {
        cout << "Error: clGetKernelWorkGroupInfo() returned a max work group "
                "size of zero!"
             << endl;
        exit(1);
    }
    ...
    if (lwsize > gwsize) {
        ...
        if (VERBOSE)
            printf(
                "Max local threads is %zu. Using %zu for local work size. \n",
...
void OpenCL::buildKernel() {
    ...
    program = clCreateProgramWithSource(
        ...
        NULL,  // Number of chars in kernel src. NULL means src is
               // null-terminated.
...
void OpenCL::getDevices(cl_device_type deviceType) {
    ...
    if (VERBOSE) {
        ...
        for (int i = 0; i < platforms_n; i++) {
            char buffer[10240];
            printf("  -- %d --\n", i);
            clGetPlatformInfo(platform_id[i], CL_PLATFORM_PROFILE, 10240,
                              buffer, NULL);
            printf("  PROFILE = %s\n", buffer);
            clGetPlatformInfo(platform_id[i], CL_PLATFORM_VERSION, 10240,
                              buffer, NULL);
            printf("  VERSION = %s\n", buffer);
            clGetPlatformInfo(platform_id[i], CL_PLATFORM_NAME, 10240, buffer,
                              NULL);
            printf("  NAME = %s\n", buffer);
            clGetPlatformInfo(platform_id[i], CL_PLATFORM_VENDOR, 10240, buffer,
                              NULL);
            printf("  VENDOR = %s\n", buffer);
            clGetPlatformInfo(platform_id[i], CL_PLATFORM_EXTENSIONS, 10240,
                              buffer, NULL);
            printf("  EXTENSIONS = %s\n", buffer);
        }
    }
    ...
    if (VERBOSE) {
        ...
        for (int i = 0; i < devices_n; i++) {
            ...
            cl_ulong buf_ulong;
            printf("  -- %d --\n", i);
            clGetDeviceInfo(device_id[i], CL_DEVICE_NAME, sizeof(buffer),
                            buffer, NULL);
            printf("  DEVICE_NAME = %s\n", buffer);
            clGetDeviceInfo(device_id[i], CL_DEVICE_VENDOR, sizeof(buffer),
                            buffer, NULL);
            printf("  DEVICE_VENDOR = %s\n", buffer);
            clGetDeviceInfo(device_id[i], CL_DEVICE_VERSION, sizeof(buffer),
                            buffer, NULL);
            printf("  DEVICE_VERSION = %s\n", buffer);
            clGetDeviceInfo(device_id[i], CL_DRIVER_VERSION, sizeof(buffer),
                            buffer, NULL);
