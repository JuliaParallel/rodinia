...
// entire
// solver to CUDA code (to remove some of the operational overheads such as
// kernel launches and data transfer in CUDA) we used a simpler solver, from
// Mathematics
// Source Library, and tailored it to our needs. The parallelism in the cardiac
// myocyte model is on a very fine-grained level, close to that of ILP,
// therefore it is very hard
// to exploit as DLP or TLB in CUDA code. We were able to divide the model into
// 4 individual groups that run in parallel. However, even that is not enough
// work to
// compensate for some of the CUDA thread launch and data transfer overheads
// which resulted in performance worse than that of single-threaded C code.
// Speedup in
// this code could be achieved only if a customizable accelerator such as FPGA
// was used for evaluation of the model itself. We also approached the
// application from
// another angle and allowed it to run several concurrent simulations, thus
// turning it into an embarrassingly parallel problem. This version of the code
// is also useful for
...
// depends on
// the number of concurrent simulations and it saturates around 300 simulations
// with the speedup of about 10x.
...
// c) kernel launch overhead (kernel needs to be finished every time model is
// evaluated as it is the only way to synchronize threads in different blocks)
...
// 3) GPU version has an issue with memory allocation that has not been resolved
...
#include "./myocyte.h" // (in directory)
...
int main(int argc, char *argv[]) {
            ...
            if (strcmp(argv[cur_arg], "-time") == 0) {
                ...
                if (argc >= cur_arg + 1) {
                    ...
                    if (isInteger(argv[cur_arg + 1]) == 1) {
                        ...
                        if (xmax < 0) {
                            printf("ERROR: Wrong value to -time argument, "
                                   "cannot be <=0\n");
                            return 0;
                        }
                        cur_arg = cur_arg + 1;
                    ...
                    // value is not a number
                        ...
                        printf(
                            "ERROR: Value to -time argument in not a number\n");
                        return 0;
                    }
                }
                // value not provided
    ...
    if (workload == 1) {
        ...
        if (memory > 1000000000) {
            printf("ERROR: trying to allocate more than 1.0GB of memory, "
                   "decrease workload and span parameters or change memory "
                   "parameter\n");
            return 0;
        }
        ...
        // y
        for (i = 0; i < workload; i++) {
            read_file("../../data/myocyte/y.txt", y[i][0], EQUATIONS, 1, 0);
        }

        // params
        for (i = 0; i < workload; i++) {
            read_file("../../data/myocyte/params.txt", params[i], PARAMETERS, 1,
                      0);
        }
