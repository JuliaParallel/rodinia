...
//(embedded_fehlberg_7_8) were adapted to work with a set of equations, not just
//one like in original version.
...
//	5) The appropriate amount of memory is previousely allocated for that
//range (y).
...
int solver(fp **y, fp *x, int xmax, fp *params, fp *com,
           ...
           cl_mem d_initvalu, cl_mem d_finavalu, cl_mem d_params, cl_mem d_com,

           cl_command_queue command_queue, cl_kernel kernel,

           long long *timecopyin, long long *timecopykernel,
    ...
    for (k = 1; k <= xmax; k++) { // start after initial value
        ...
        for (j = 0; j < ATTEMPTS; j++) {
            ...
            embedded_fehlberg_7_8(x[k], h, y[k - 1], y[k], err, params, com,
                                  ...
                                  timecopyin, timecopykernel, timecopyout);
