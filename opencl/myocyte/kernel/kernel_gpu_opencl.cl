...
//======================================================================================================================================================150
//	DEFINE (need to define because cannot include ../common.h file for some
//reason
//======================================================================================================================================================150
...
void kernel_ecc(fp timeinst, __global fp *d_initvalu, __global fp *d_finavalu,
                int valu_offset, __global fp *d_params) {
    ...
    // fp junctionLength;																// junc length
    // [um]
    // fp junctionRadius;																// junc radius
    // [um]
    // fp distSLcyto;																	// dist. SL to cytosol
    // [um]
    // fp distJuncSL;																	// dist. junc to SL
    // [um]
    // fp DcaJuncSL;																	// Dca junc to SL
    // [cm^2/sec]
    // fp DcaSLcyto;																	// Dca SL to cyto
    // [cm^2/sec]
    // fp DnaJuncSL;																	// Dna junc to SL
    // [cm^2/sec]
    // fp DnaSLcyto;																	// Dna SL to cyto
    // [cm^2/sec]
    fp Vcell; // [L]
    fp Vmyo;
    fp Vsr;
    ...
    // junctionLength = 160e-3;															// junc length
    // [um]
    // junctionRadius = 15e-3;																// junc radius
    // [um]
    // distSLcyto = 0.45;																	// dist. SL to cytosol
    // [um]
    // distJuncSL = 0.5;																	// dist. junc to SL
    // [um]
    // DcaJuncSL = 1.64e-6;																// Dca junc to SL
    // [cm^2/sec]
    // DcaSLcyto = 1.22e-6;																// Dca SL to cyto
    // [cm^2/sec]
    // DnaJuncSL = 1.09e-5;																// Dna junc to SL
    // [cm^2/sec]
    // DnaSLcyto = 1.79e-5;																// Dna SL to cyto
    // [cm^2/sec]
    ...
    am = 0.32 * (d_initvalu_39 + 47.13) /
         (1 - exp(-0.1 * (d_initvalu_39 + 47.13)));
    bm = 0.08 * exp(-d_initvalu_39 / 11);
    if (d_initvalu_39 >= -40) {
        ah = 0;
        aj = 0;
        bh = 1 / (0.13 * (1 + exp(-(d_initvalu_39 + 10.66) / 11.1)));
        bj = 0.3 * exp(-2.535e-7 * d_initvalu_39) /
             (1 + exp(-0.1 * (d_initvalu_39 + 32)));
    } else {
        ah = 0.135 * exp((80 + d_initvalu_39) / -6.8);
        bh = 3.56 * exp(0.079 * d_initvalu_39) +
             3.1e5 * exp(0.35 * d_initvalu_39);
        aj = (-127140 * exp(0.2444 * d_initvalu_39) -
              3.474e-5 * exp(-0.04391 * d_initvalu_39)) *
             (d_initvalu_39 + 37.78) /
             (1 + exp(0.311 * (d_initvalu_39 + 79.23)));
        bj = 0.1212 * exp(-0.01052 * d_initvalu_39) /
             (1 + exp(-0.1378 * (d_initvalu_39 + 40.14)));
    }
    ...
    I_Na_junc = Fjunc * GNa * pow(d_initvalu_1, 3) * d_initvalu_2 *
    ...
    fnak = 1 / (1 + 0.1245 * exp(-0.1 * d_initvalu_39 * FoRT) +
                0.0365 * sigma * exp(-d_initvalu_39 * FoRT));
    I_nak_junc = Fjunc * IbarNaK * fnak * Ko /
                 (1 + pow((KmNaip / d_initvalu_32), 4)) / (Ko + KmKo);
    I_nak_sl = Fsl * IbarNaK * fnak * Ko /
               (1 + pow((KmNaip / d_initvalu_33), 4)) / (Ko + KmKo);
    ...
    tauxr = 1 / (0.00138 * (d_initvalu_39 + 7) /
                     (1 - exp(-0.123 * (d_initvalu_39 + 7))) +
                 6.1e-4 * (d_initvalu_39 + 10) /
                     (exp(0.145 * (d_initvalu_39 + 10)) - 1));
    ...
    tauxs = 1 / (7.19e-5 * (d_initvalu_39 + 30) /
                     (1 - exp(-0.148 * (d_initvalu_39 + 30))) +
                 1.31e-4 * (d_initvalu_39 + 30) /
                     (exp(0.0687 * (d_initvalu_39 + 30)) - 1));
    d_finavalu[offset_13] = (xsss - d_initvalu_13) / tauxs;
    I_ks_junc =
        Fjunc * gks_junc * pow(d_initvalu_12, 2) * (d_initvalu_39 - eks);
    ...
    I_tos = GtoSlow * d_initvalu_8 * (d_initvalu_9 + 0.5 * d_initvalu_40) *
            (d_initvalu_39 - ek); // [uA/uF]
    ...
    // I_ClCa: Ca-activated Cl Current, I_Clbk: background Cl Current
    I_ClCa_junc =
        Fjunc * GClCa / (1 + KdClCa / d_initvalu_36) * (d_initvalu_39 - ecl);
    I_ClCa_sl =
        Fsl * GClCa / (1 + KdClCa / d_initvalu_37) * (d_initvalu_39 - ecl);
    I_ClCa = I_ClCa_junc + I_ClCa_sl;
    I_Clbk = GClB * (d_initvalu_39 - ecl);

    // I_Ca: L-type Calcium Current
    dss = 1 / (1 + exp(-(d_initvalu_39 + 14.5) / 6.0));
    taud = dss * (1 - exp(-(d_initvalu_39 + 14.5) / 6.0)) /
           (0.035 * (d_initvalu_39 + 14.5));
    fss = 1 / (1 + exp((d_initvalu_39 + 35.06) / 3.6)) +
          0.6 / (1 + exp((50 - d_initvalu_39) / 20));
    ...
    d_finavalu[offset_6] = 1.7 * d_initvalu_36 * (1 - d_initvalu_6) -
                           11.9e-3 * d_initvalu_6; // fCa_junc
    d_finavalu[offset_7] = 1.7 * d_initvalu_37 * (1 - d_initvalu_7) -
                           11.9e-3 * d_initvalu_7; // fCa_sl

    //
    ibarca_j =
        pCa * 4 * (d_initvalu_39 * Frdy * FoRT) *
    ...
    I_Ca_junc = (Fjunc_CaL * ibarca_j * d_initvalu_4 * d_initvalu_5 *
                 (1 - d_initvalu_6) * pow(Q10CaL, Qpow)) *
                0.45;
    I_Ca_sl = (Fsl_CaL * ibarca_sl * d_initvalu_4 * d_initvalu_5 *
               (1 - d_initvalu_7) * pow(Q10CaL, Qpow)) *
              0.45;
    ...
    s2_junc =
        exp((nu - 1) * d_initvalu_39 * FoRT) * pow(Nao, 3) * d_initvalu_36;
    s3_junc = (KmCai * pow(Nao, 3) * (1 + pow((d_initvalu_32 / KmNai), 3)) +
               pow(KmNao, 3) * d_initvalu_36 +
               pow(KmNai, 3) * Cao * (1 + d_initvalu_36 / KmCai) +
               KmCao * pow(d_initvalu_32, 3) + pow(d_initvalu_32, 3) * Cao +
               pow(Nao, 3) * d_initvalu_36) *
    ...
    s3_sl = (KmCai * pow(Nao, 3) * (1 + pow((d_initvalu_33 / KmNai), 3)) +
             pow(KmNao, 3) * d_initvalu_37 +
             pow(KmNai, 3) * Cao * (1 + d_initvalu_37 / KmCai) +
             KmCao * pow(d_initvalu_33, 3) + pow(d_initvalu_33, 3) * Cao +
             pow(Nao, 3) * d_initvalu_37) *
            (1 + ksat * exp((nu - 1) * d_initvalu_39 * FoRT));
    ...
    // I_pca: Sarcolemmal Ca Pump Current
    I_pca_junc = Fjunc * pow(Q10SLCaP, Qpow) * IbarSLCaP *
                 pow(d_initvalu_36, (fp)(1.6)) /
                 (pow(KmPCa, (fp)(1.6)) + pow(d_initvalu_36, (fp)(1.6)));
    I_pca_sl = Fsl * pow(Q10SLCaP, Qpow) * IbarSLCaP *
               pow(d_initvalu_37, (fp)(1.6)) /
    ...
    MinSR = 1;
    kCaSR =
        MaxSR - (MaxSR - MinSR) / (1 + pow(ec50SR / d_initvalu_31, (fp)(2.5)));
    koSRCa = koCa / kCaSR;
    kiSRCa = kiCa * kCaSR;
    ...
    d_finavalu[offset_15] =
        (koSRCa * pow(d_initvalu_36, 2) * d_initvalu_14 - kom * d_initvalu_15) -
    ...
    J_serca = pow(Q10SRCaP, Qpow) * Vmax_SRCaP *
              (pow((d_initvalu_38 / Kmf), hillSRCaP) -
               pow((d_initvalu_31 / Kmr), hillSRCaP)) /
              (1 + pow((d_initvalu_38 / Kmf), hillSRCaP) +
               pow((d_initvalu_31 / Kmr), hillSRCaP));
    ...
    I_Na_tot_junc = I_Na_junc + I_nabk_junc + 3 * I_ncx_junc + 3 * I_nak_junc +
                    I_CaNa_junc; // [uA/uF]
    I_Na_tot_sl = I_Na_sl + I_nabk_sl + 3 * I_ncx_sl + 3 * I_nak_sl +
                  I_CaNa_sl; // [uA/uF]
...
void kernel_cam(fp timeinst, __global fp *d_initvalu, __global fp *d_finavalu,
                int valu_offset, __global fp *d_params, int params_offset,
                __global fp *d_com, int com_offset, fp Ca) {
...
__kernel void kernel_gpu_opencl(int timeinst, __global fp *d_initvalu,
                                __global fp *d_finavalu, __global fp *d_params,
                                __global fp *d_com) {
    ...
    // indexes
    // bx = blockIdx.x;																// get current horizontal block index
    // (0-n)
    // tx = threadIdx.x;																// get current horizontal thread index
    // (0-n)
    ...
    // limit to useful threads
        ...
        if (tx == 0) { // only 1 thread runs it, since its a sequential code
            ...
            kernel_cam(timeinst, d_initvalu, d_finavalu, valu_offset, d_params,
                       params_offset, d_com, com_offset, CaDyad);
            ...
            // cam function for Dyad
            kernel_cam(timeinst, d_initvalu, d_finavalu, valu_offset, d_params,
                       params_offset, d_com, com_offset, CaSL);
            ...
            // cam function for Dyad
            kernel_cam(timeinst, d_initvalu, d_finavalu, valu_offset, d_params,
                       params_offset, d_com, com_offset, CaCyt);
        }
    }
