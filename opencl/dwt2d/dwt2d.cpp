...
// CreateProgram, CreateCommandQueue, CreateMemBuffer, and Cleanup
...
cl_context CreateContext() {
    ...
    // In this example, first platform is a CPU, the second one is a GPU. we
    // just choose the first available device.
    cl_context_properties contextProperties[] = {
        CL_CONTEXT_PLATFORM, (cl_context_properties)platformIds[1], 0};
...
//
cl_program CreateProgram(cl_context context, cl_device_id cldevice,
                         const char *fileName) {
    cl_int errNum;
    cl_program program;
...
void rgbToComponents(cl_mem d_r, cl_mem d_g, cl_mem d_b, unsigned char *h_src,
    ...
    cl_mem cl_d_src;
    cl_d_src = clCreateBuffer(context, CL_MEM_READ_ONLY | CL_MEM_COPY_HOST_PTR,
                              pixels * 3, h_src, &errNum);
    ...
    errNum |=
        clSetKernelArg(c_CopySrcToComponents, 3, sizeof(cl_mem), &cl_d_src);
    errNum |= clSetKernelArg(c_CopySrcToComponents, 4, sizeof(int), &pixels);
    // fatal_CL(errNum, __LINE__);

    errNum =
        clEnqueueNDRangeKernel(commandQueue, c_CopySrcToComponents, 1, NULL,
...
void bwToComponent(cl_mem d_c, unsigned char *h_src, int width, int height) {
    ...
    errNum |=
        clSetKernelArg(c_CopySrcToComponent, 1, sizeof(cl_mem), &cl_d_src);
    errNum |= clSetKernelArg(c_CopySrcToComponent, 2, sizeof(int), &pixels);
    // fatal_CL(errNum, __LINE__);

    errNum =
        clEnqueueNDRangeKernel(commandQueue, c_CopySrcToComponent, 1, NULL,
                               globalWorkSize, localWorkSize, 0, NULL, NULL);
...
/// Only computes optimal number of sliding window steps, number of threadblocks
/// and then lanches the 5/3 FDWT kernel.
...
void launchFDWT53Kernel(int WIN_SX, int WIN_SY, cl_mem in, cl_mem out, int sx,
                        int sy) {
    // compute optimal number of steps of each sliding window
    // cuda_dwt called a function divRndUp from namespace cuda_gwt. this
    // function takes n and d, "return (n / d) + ((n % d) ? 1 : 0);"
    //

    const int steps = (sy / (15 * WIN_SY)) + ((sy % (15 * WIN_SY)) ? 1 : 0);

    int gx = (sx / WIN_SX) + ((sx % WIN_SX) ? 1 : 0); // use function
                                                      // divRndUp(n, d){return
                                                      // (n / d) + ((n % d) ? 1
                                                      // : 0);}
    ...
    errNum =
        clEnqueueNDRangeKernel(commandQueue, kl_fdwt53Kernel, 2, NULL,
                               globalWorkSize, localWorkSize, 0, NULL, NULL);
    // fatal_CL(errNum, __LINE__);
...
void memCopy(cl_mem dest, cl_mem src, const size_t sx, const size_t sy) {
    ...
    errNum = clEnqueueCopyBuffer(commandQueue, src, dest, 0, 0,
                                 sx * sy * sizeof(int), 0, NULL, NULL);
    // fatal_CL (errNum, __LINE__);
}
...
///
// in dwt.cu
int nStage2dDWT(cl_mem in, cl_mem out, cl_mem backup, int pixWidth,
                int pixHeight, int stages, bool forward) {
    ...
    // rdwt means rdwt53(can be found in file rdwt53.cu) which has not been
    // defined
...
int writeLinear(cl_mem component, int pixWidth, int pixHeight,
    ...
    errNum = clEnqueueReadBuffer(commandQueue, component, CL_TRUE, 0, size,
                                 gpu_output, 0, NULL, NULL);
    // fatal_CL(errNum, __LINE__);
...
// in file dwt.cu
int writeNStage2DDWT(cl_mem component, int pixWidth, int pixHeight, int stages,
                     const char *filename, const char *suffix) {
    struct band {
        int dimX;
        int dimY;
    ...
    errNum = clEnqueueReadBuffer(commandQueue, component, CL_TRUE, 0, size, src,
                                 0, NULL, NULL);
    // fatal_CL(errNum, __LINE__);
    ...
    for (i = 0; i < bandDims[stages - 1].LL.dimY; i++) {
        memcpy(dst + i * pixWidth, src + i * bandDims[stages - 1].LL.dimX,
               size);
    }
...
void processDWT(struct dwt *d, int forward, int writeVisual) {
    ...
    if (d->components == 3) {
        ...
        rgbToComponents(cl_c_r, cl_c_g, cl_c_b, d->srcImg, d->pixWidth,
                        d->pixHeight);


        // Compute DWT and always store int file

        nStage2dDWT(cl_c_r, cl_c_r_out, cl_backup, d->pixWidth, d->pixHeight,
                    d->dwtLvls, forward);
        nStage2dDWT(cl_c_g, cl_c_g_out, cl_backup, d->pixWidth, d->pixHeight,
                    d->dwtLvls, forward);
        nStage2dDWT(cl_c_b, cl_c_b_out, cl_backup, d->pixWidth, d->pixHeight,
                    d->dwtLvls, forward);
...
*/ // ---------test----------
        ...
        // Store DWT to file
        if (writeVisual) {
            writeNStage2DDWT(cl_c_r_out, d->pixWidth, d->pixHeight, d->dwtLvls,
                             d->outFilename, ".r");
            writeNStage2DDWT(cl_c_g_out, d->pixWidth, d->pixHeight, d->dwtLvls,
                             d->outFilename, ".g");
            writeNStage2DDWT(cl_c_b_out, d->pixWidth, d->pixHeight, d->dwtLvls,
                             d->outFilename, ".b");

        } else {
            writeLinear(cl_c_r_out, d->pixWidth, d->pixHeight, d->outFilename,
                        ".r");
            writeLinear(cl_c_g_out, d->pixWidth, d->pixHeight, d->outFilename,
                        ".g");
            writeLinear(cl_c_b_out, d->pixWidth, d->pixHeight, d->outFilename,
                        ".b");
        }
#endif
...
int main(int argc, char **argv) {
    ...
    c_CopySrcToComponents =
        clCreateKernel(program, "c_CopySrcToComponents", NULL);
    if (c_CopySrcToComponents == NULL) {
        std::cerr << "Failed to create kernel" << std::endl;
    }

    c_CopySrcToComponent =
        clCreateKernel(program, "c_CopySrcToComponent", NULL);
    ...
    // DWT
    // Create memory objects, Set arguments for kernel functions, Queue the
    // kernel up for execution across the array, Read the output buffer back to
    // the Host, Output the result buffer
