...
int divRndUp(int n, int d) { return (n / d) + ((n % d) ? 1 : 0); }


/* Store 3 RGB float components */
/*void storeComponents(__global float *d_r, __global float *d_g, __global float
*d_b, __global const float r, __global const float g, __global const float b,
int pos)
{
...
void storeComponents(__global int *d_r, __global int *d_g, __global int *d_b,
                     int r, int g, int b, int pos) {
    d_r[pos] = r - 128;
    d_g[pos] = g - 128;
    d_b[pos] = b - 128;
}
...
/*__kernel void storeComponent(__global float *d_c, __global const float c, int
pos)
{
...
void storeComponent(__global int *d_c, const int c, int pos) {
    d_c[pos] = c - 128;
}


// Copy img src data into three separated component buffers
__kernel void c_CopySrcToComponents(__global int *d_r, __global int *d_g,
                                    __global int *d_b,
...
int initialize_PixelIO(struct VerticalDWTPixelIO *pIO, bool CHECK,
                       const int sizeX, const int sizeY, int firstX,
                       int firstY) {
    pIO->CHECKED = CHECK;
...
void init_PixelLoader(struct VerticalDWTPixelLoader *loader, const int sizeX,
                      const int sizeY, int firstX, const int firstY,
                      struct VerticalDWTPixelIO *pIO, bool CHECK) {
    mirror(&firstX, sizeX);
    loader->last =
        initialize_PixelIO(pIO, CHECK, sizeX, sizeY, firstX, firstY) - sizeX;
}
...
int loadFrom(struct VerticalDWTPixelLoader *pLoader,
             __global const int *const input, struct VerticalDWTPixelIO *pIO,
             int CHECK) {
...
int initialize_BandIO(struct VerticalDWTBandIO *bandIO, const int imageSizeX,
                      const int imageSizeY, int firstX, int firstY) {
    ...
    if (bandIO->CHECKED) {
        bandIO->end = columnOffset + (imageSizeY / 2) * verticalStride +
                      (imageSizeY & 1) * bandIO->strideLowToHigh;
    } else {
        bandIO->end = 0;
    }
...
int saveAndUpdate(struct VerticalDWTBandWriter *writer, bool CHECK,
                  struct VerticalDWTBandIO *bandIO, __global int *const output,
                  __local int *item, int *stride) {
...
void init_BandWriter(struct VerticalDWTBandWriter *writer,
                     struct VerticalDWTBandIO *bandIO, const int imageSizeX,
                     const int imageSizeY, const int firstX, const int firstY) {
    if (firstX < imageSizeX) {
        writer->next =
            initialize_BandIO(bandIO, imageSizeX, imageSizeY, firstX, firstY);
    } else {
...
int writeLowInto(struct VerticalDWTBandWriter *writer,
                 struct VerticalDWTBandIO *bandIO, __global int *const output,
                 __local int *primary) {
    return saveAndUpdate(writer, writer->CHECKED, bandIO, output, primary,
                         &(bandIO->strideLowToHigh));
}

int writeHighInto(struct VerticalDWTBandWriter *writer,
                  struct VerticalDWTBandIO *bandIO, __global int *const output,
                  __local int *other) {
    return saveAndUpdate(writer, writer->CHECKED, bandIO, output, other,
                         &(bandIO->strideHighToLow));
}
...
void horizontalStep(__local struct TransformBuffer *buffer, const int count,
                    const int prevOffset, const int midOffset,
                    const int nextOffset, int flag) {
    const int STEPS = count / buffer->SIZE_X;
    const int finalCount = count % buffer->SIZE_X;
    const int finalOffset = count - finalCount;
    for (int i = 0; i < STEPS; i++) {
        const int previous =
            buffer->data[prevOffset + i * buffer->SIZE_X + get_local_id(0)];
        const int next =
            buffer->data[nextOffset + i * buffer->SIZE_X + get_local_id(0)];
        __local int *center =
            &(buffer->data[midOffset + i * buffer->SIZE_X + get_local_id(0)]);
        if (flag == 0) {
    ...
    if (get_local_id(0) < finalCount) {
        const int previous =
            buffer->data[prevOffset + finalOffset + get_local_id(0)];
        const int next =
            buffer->data[nextOffset + finalOffset + get_local_id(0)];
        __local int *center =
            &(buffer->data[midOffset + finalOffset + get_local_id(0)]);
...
void forEachHorizontalOdd(__local struct TransformBuffer *buffer,
                          const int firstLine, const int numLines, int flag) {
...
void forEachHorizontalEven(__local struct TransformBuffer *buffer,
    ...
    const int prevOffset =
        firstLine * buffer->VERTICAL_STRIDE + buffer->ODD_OFFSET;
    const int nextOffset = prevOffset + 1;
...
void forEachVerticalOdd(__local struct TransformBuffer *buffer,
                        const int columnOffset, int flag) {
    int steps = (buffer->SIZE_Y - 1) / 2;
    for (int i = 0; i < steps; i++) {
        int row = i * 2 + 1;
        int prev =
            buffer->data[columnOffset + (row - 1) * buffer->VERTICAL_STRIDE];
        int next =
            buffer->data[columnOffset + (row + 1) * buffer->VERTICAL_STRIDE];

        if (flag == 0) {
            buffer->data[columnOffset + row * buffer->VERTICAL_STRIDE] -=
                (prev + next) / 2;
        } else if (flag == 1) {
            // buffer->data[columnOffset + row * buffer->VERTICAL_STRIDE] +=
            // (prev + next + 2) /4;
...
void forEachVerticalEven(__local struct TransformBuffer *buffer,
    ...
    if (buffer->SIZE_Y > 3) {
        ...
        for (i = 0; i < steps; i++) {
            ...
            int prev =
                buffer
                    ->data[columnOffset + (row - 1) * buffer->VERTICAL_STRIDE];
            int next =
                buffer
                    ->data[columnOffset + (row + 1) * buffer->VERTICAL_STRIDE];
...
int getColumnOffset(int columnIndex, __local struct TransformBuffer *buffer) {
...
void initColumn(__local struct FDWT53 *fdwt53, struct FDWT53Column *column,
                bool CHECKED, __global const int *const input, const int sizeX,
                const int sizeY, const int colIndex, const int firstY,
                struct VerticalDWTPixelIO *pIO) {
    ...
    if (get_group_id(1) == 0) {
        ...
        init_PixelLoader(&(column->loader), sizeX, sizeY, firstX, firstY, pIO,
                         CHECKED);
        column->pixel2 =
            loadFrom(&(column->loader), input, pIO, CHECKED); // loaded pixel #0
        column->pixel1 =
            loadFrom(&(column->loader), input, pIO, CHECKED); // loaded pixel #1
        column->pixel0 =
            loadFrom(&(column->loader), input, pIO, CHECKED); // loaded pixel #2
        init_PixelLoader(&(column->loader), sizeX, sizeY, firstX, firstY + 1,
                         pIO, CHECKED);
    } else {
        init_PixelLoader(&(column->loader), sizeX, sizeY, firstX, firstY - 2,
                         pIO, CHECKED);
        column->pixel0 =
            loadFrom(&(column->loader), input, pIO, CHECKED); // loaded pixel #0
        column->pixel1 =
...
void loadAndVerticallyTransform(__local struct FDWT53 *fdwt53,
                                struct FDWT53Column *column, bool CHECKED,
    ...
    for (int i = 3; i < (3 + fdwt53->WIN_SIZE_Y); i++) {
        fdwt53->buffer.data[column->offset + i * fdwt53->STRIDE] =
            loadFrom(&(column->loader), input, pIO, CHECKED);
    }


    column->pixel0 =
        fdwt53->buffer
            .data[column->offset + (fdwt53->WIN_SIZE_Y + 0) * fdwt53->STRIDE];
    column->pixel1 =
        fdwt53->buffer
            .data[column->offset + (fdwt53->WIN_SIZE_Y + 1) * fdwt53->STRIDE];
    column->pixel2 =
        fdwt53->buffer
            .data[column->offset + (fdwt53->WIN_SIZE_Y + 2) * fdwt53->STRIDE];
...
void transform(__local struct FDWT53 *fdwt53, bool CHECK_LOADS,
               bool CHECK_WRITES, __global const int *const in,
               __global int *out, const int sizeX, const int sizeY,
               const int winSteps) {
    // info about one main and one boundary columns processed by this thread
    struct FDWT53Column column;
    column.CHECKED_LOADER = CHECK_LOADS;
    struct VerticalDWTPixelIO pIO;
    struct FDWT53Column boundaryColumn;
    boundaryColumn.CHECKED_LOADER = CHECK_LOADS;
    ...
    initColumn(fdwt53, &column, CHECK_LOADS, in, sizeX, sizeY, get_local_id(0),
               firstY, &pIO);
    ...
    if (get_local_id(0) < 3) {
        ...
        const int colId = get_local_id(0) +
                          ((get_local_id(0) == 0) ? fdwt53->WIN_SIZE_X : -3);

        // initialize the column
        initColumn(fdwt53, &boundaryColumn, CHECK_LOADS, in, sizeX, sizeY,
                   colId, firstY, &pIO_b);
    }

    // index of column which will be written into output by this thread
    const int outColumnIndex =
        (get_local_id(0) * 2) -
        (fdwt53->WIN_SIZE_X - 1) * (get_local_id(0) / (fdwt53->WIN_SIZE_X / 2));

    // offset of column which will be written by this thread into output
    const int outColumnOffset =
        getColumnOffset(outColumnIndex, &(fdwt53->buffer));

    // initialize output writer for this thread
    const int outputFirstX =
        get_group_id(0) * fdwt53->WIN_SIZE_X + outColumnIndex;
    struct VerticalDWTBandWriter writer;
    writer.CHECKED = CHECK_WRITES;
    struct VerticalDWTBandIO bandIO;
    bandIO.CHECKED = CHECK_WRITES;
    ...
    for (int w = 0; w < winSteps; w++) {
        ...
        if (get_local_id(0) < 3) {
            loadAndVerticallyTransform(fdwt53, &boundaryColumn, CHECK_LOADS, in,
                                       &pIO_b);
        }

        barrier(CLK_LOCAL_MEM_FENCE);

        int flag = 0; // flag = 0 execute Forward53Predict, flag = 1 execute
                      // Forward53Update
        ...
        for (int r = 2; r < (2 + fdwt53->WIN_SIZE_Y); r += 2) {
            writeLowInto(
                &writer, &bandIO, out,
                &(fdwt53->buffer.data[outColumnOffset +
                                      r * fdwt53->buffer.VERTICAL_STRIDE]));
            writeHighInto(
                &writer, &bandIO, out,
                &(fdwt53->buffer
                      .data[outColumnOffset +
                            (r + 1) * fdwt53->buffer.VERTICAL_STRIDE]));
        }
...
void Forward53Predict(const int p, __global int *c, const int n) {
    *c -= (p + n) / 2;
}
// Forward 5/3 DWT update operation.
void Forward53Update(const int p, __global int *c, const int n) {
    *c += (p + n + 2) / 4;
}


__kernel void cl_fdwt53Kernel(__global const int *const in, __global int *out,
                              const int sx, const int sy, const int steps,
                              int WIN_SIZE_X, int WIN_SIZE_Y) {
    ...
    fdwt53.buffer.VERTICAL_STRIDE =
        BOUNDARY_X + (fdwt53.buffer.SIZE_X / 2); // BOUNDARY = 2
    fdwt53.buffer.SHM_BANKS =
        32; // SHM_BANKS = ((__CUDA_ARCH__ >= 200) ? 32 : 16)
    fdwt53.buffer.BUFFER_SIZE =
        fdwt53.buffer.VERTICAL_STRIDE * fdwt53.buffer.SIZE_Y;
    fdwt53.buffer.PADDING =
        fdwt53.buffer.SHM_BANKS -
        ((fdwt53.buffer.BUFFER_SIZE + fdwt53.buffer.SHM_BANKS / 2) %
         fdwt53.buffer.SHM_BANKS);
    fdwt53.buffer.ODD_OFFSET =
        fdwt53.buffer.BUFFER_SIZE + fdwt53.buffer.PADDING;
