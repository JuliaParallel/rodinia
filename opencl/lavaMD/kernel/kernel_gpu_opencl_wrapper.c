...
void kernel_gpu_opencl_wrapper(par_str par_cpu, dim_str dim_cpu,
    ...
    cl_mem d_box_gpu;
    d_box_gpu = clCreateBuffer(context, CL_MEM_READ_WRITE, dim_cpu.box_mem,
                               NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_mem d_rv_gpu;
    d_rv_gpu = clCreateBuffer(context, CL_MEM_READ_WRITE, dim_cpu.space_mem,
                              NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_mem d_fv_gpu;
    d_fv_gpu = clCreateBuffer(context, CL_MEM_READ_WRITE, dim_cpu.space_mem,
                              NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_rv_gpu, 1, 0,
                                 dim_cpu.space_mem, rv_cpu, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_fv_gpu, 1, 0,
                                 dim_cpu.space_mem, fv_cpu, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // (enable for testing purposes only - prints some range of output, make
    // sure not to initialize input in main.c with random numbers for comparison
    // across runs)
    // int g;
    // int offset = 395;
    // for(g=0; g<10; g++){
    // printf("%f, %f, %f, %f\n", fv_cpu[offset+g].v, fv_cpu[offset+g].x,
    // fv_cpu[offset+g].y, fv_cpu[offset+g].z);
    // }
