...
//========================================================================================================================================================================================================200
//	INCLUDE/DEFINE (had to bring from ./../lavaMD.h here because feature of
//including headers in clBuildProgram does not work for some reason)
//========================================================================================================================================================================================================200

// #include <lavaMD.h>						// (in the directory SOMEHOW
// known to the OpenCL compiler function)

#define fp float

#define NUMBER_PAR_PER_BOX                                                     \
    100 // keep this low to allow more blocks that share shared memory to run
        // concurrently, code does not work for larger than 110, more speedup
        // can be achieved with larger number and no shared memory used

#define NUMBER_THREADS                                                         \
    128 // this should be roughly equal to NUMBER_PAR_PER_BOX for best
        // performance

#define DOT(A, B) ((A.x) * (B.x) + (A.y) * (B.y) + (A.z) * (B.z)) // STABLE

//===============================================================================================================================================================================================================200
//	STRUCTURES (had to bring from ./../lavaMD.h here because feature of
//including headers in clBuildProgram does not work for some reason)
//===============================================================================================================================================================================================================200
...
__kernel void kernel_gpu_opencl(par_str d_par_gpu, dim_str d_dim_gpu,
        ...
        for (k = 0; k < (1 + d_box_gpu[bx].nn); k++) {
            ...
            while (wtx < NUMBER_PAR_PER_BOX) {
                ...
                for (j = 0; j < NUMBER_PAR_PER_BOX; j++) {
                    ...
                    // (disable the section below only if wanting to use shared
                    // memory)
                    // r2 = d_rv_gpu[first_i+wtx].v + d_rv_gpu[first_j+j].v -
                    // DOT(d_rv_gpu[first_i+wtx],d_rv_gpu[first_j+j]);
                    // u2 = a2*r2;
                    // vij= exp(-u2);
                    // fs = 2*vij;
                    ...
                    // (enable the section below only if wanting to use shared
                    // memory)
