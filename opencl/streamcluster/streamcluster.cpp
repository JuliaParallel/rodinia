...
//#define PROFILE_TMP 					// comment this out to
//disable instrumentation code
//#define ENABLE_THREADS  // comment this out to disable threads
//#define INSERT_WASTE 		//uncomment this to insert waste computation
//into dist function
    ...
    if (pid !=
        ...
        for (i = 1; i < points->num; i++) {
            bool to_open = ((float)lrand48() / (float)INT_MAX) <
                           (points->p[i].cost / z); //--cambine: what standard?
            if (to_open) {
                (*kcenter)++;
#ifdef ENABLE_THREADS
                pthread_mutex_lock(&mutex);
...
#endif
                for (int k = k1; k < k2; k++) { //--cambine: for a new open,
                                                //compute new cost and center.
                    float distance =
        ...
        for (i = 0; i < iter; i++) {
            ...
            change +=
                pgain(feasible[x], points, z, k, kmax, is_center, center_table,
                      switch_membership, &serial, &cpu_gpu_memcpy, &memcpy_back,
                      &gpu_malloc, &kernel);
        }
        cost -= change;
#ifdef PRINTINFO
...
float pkmedian(Points *points, long kmin, long kmax, long *kfinal, int pid,
    ...
    if (pid == 0)
        shuffle(points); //--cambine: why need shuffle?
...
#define __PARSEC_XSTRING(x) __PARSEC_STRING(x)
    ...
    fprintf(fp, "%lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf, %lf\n", time_FL,
            cpu_gpu_memcpy, memcpy_back, kernel, gpu_malloc, gpu_free, 0.0);
    fclose(fp);
#endif
    _clStatistics();
#ifdef ENABLE_PARSEC_HOOKS
