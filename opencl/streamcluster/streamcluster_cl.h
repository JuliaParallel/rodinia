...
float pgain(long x, Points *points, float z, long int *numcenters, int kmax,
            ...
            double *gpu_malloc, double *kernel) {
    float gl_cost = 0;
    try {
#ifdef PROFILE_TMP
        double t1 = gettime();
        ...
        *serial += t2 - t1;
        ...
        /***** initial memory allocation and preparation for transfer : execute
         * once -1 *****/
        if (c == 0) {
#ifdef PROFILE_TMP
            double t3 = gettime();
#endif
            coord_h = (float *)malloc(num * dim *
                                      sizeof(float)); // coordinates (host)
            gl_lower = (float *)malloc(kmax * sizeof(float));
            work_mem_h = (float *)_clMallocHost(kmax * num * sizeof(float));
            p_h = (Point_Struct *)malloc(
                num * sizeof(Point_Struct)); // by cambine: not compatibal with
                                             // original Point

            // prepare mapping for point coordinates
            //--cambine: what's the use of point coordinates? for computing
            //distance.
            for (int i = 0; i < dim; i++) {
                for (int j = 0; j < num; j++)
        ...
        *cpu_gpu_memcpy += t8 - t7;
        ...
        _clInvokeKernel(kernel_id, num,
                        THREADS_PER_BLOCK); //--cambine: start kernel here
        _clFinish();
        ...
        *kernel += t10 - t9;
        ...
        _clMemcpyD2H(switch_membership, switch_membership_d,
                     num * sizeof(char));
