...
#include "./../srad.h" // (in the main program folder)
...
void kernel_gpu_opencl_wrapper(fp *image,  // input image
    ...
    cl_mem d_sums2;
    d_sums2 =
        clCreateBuffer(context, CL_MEM_READ_WRITE, mem_size, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_I, 1, 0, mem_size, image, 0,
                                 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_iN, 1, 0, mem_size_i, iN, 0,
                                 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    error = clEnqueueWriteBuffer(command_queue, d_iS, 1, 0, mem_size_i, iS, 0,
                                 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    error = clEnqueueWriteBuffer(command_queue, d_jE, 1, 0, mem_size_j, jE, 0,
                                 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    error = clEnqueueWriteBuffer(command_queue, d_jW, 1, 0, mem_size_j, jW, 0,
                                 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    global_work_size[0] =
        blocks_work_size *
        local_work_size[0]; // define the number of blocks in the grid
    ...
    error = clEnqueueNDRangeKernel(command_queue, extract_kernel, 1, NULL,
    ...
    //====================================================================================================100
    //	Synchronization - wait for all operations in the command queue so far to
    //finish
    //====================================================================================================100
    ...
    for (iter = 0; iter < niter;
        ...
        // launch kernel
        error = clEnqueueNDRangeKernel(command_queue, prepare_kernel, 1, NULL,
        ...
        while (blocks2_work_size != 0) {
            ...
            error = clSetKernelArg(reduce_kernel, 5, sizeof(int),
                                   (void *)&blocks2_work_size);
            if (error != CL_SUCCESS)
                fatal_CL(error, __LINE__);

            // launch kernel
            error = clEnqueueNDRangeKernel(command_queue, reduce_kernel, 1,
                                           NULL, global_work_size2,
            ...
            if (blocks2_work_size == 1) {
                ...
                blocks_x = blocks2_work_size /
                           (int)local_work_size[0]; // number of blocks
                ...
                global_work_size2[0] =
                    blocks2_work_size * (int)local_work_size[0];
            }
        }

        // copy total sums to device
        error = clEnqueueReadBuffer(command_queue, d_sums, CL_TRUE, 0,
                                    mem_size_single, &total, 0, NULL, NULL);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);

        error = clEnqueueReadBuffer(command_queue, d_sums2, CL_TRUE, 0,
        ...
        meanROI =
            total / (fp)(NeROI); // gets mean (average) value of element in ROI
    ...
    error = clEnqueueNDRangeKernel(command_queue, compress_kernel, 1, NULL,
                                   global_work_size, local_work_size, 0, NULL,
                                   NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
