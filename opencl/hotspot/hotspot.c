...
int main(int argc, char **argv) {
    ...
    // Print the chosen platform (if there are multiple platforms, choose the
    // first one)
    ...
    // Get and print the chosen device (if there are multiple devices, choose
    // the first one)
    size_t devices_size;
    error =
    ...
    cl_float *MatrixOut = (cl_float *)clEnqueueMapBuffer(
        command_queue, MatrixTemp[ret], CL_TRUE, CL_MAP_READ, 0,
    ...
    error = clEnqueueUnmapMemObject(command_queue, MatrixTemp[ret],
