...
__kernel void initialize_variables(__global float *variables,
                                   __constant float *ff_variable, int nelr) {
...
__kernel void compute_step_factor(__global float *variables,
                                  ...
                                  __global float *step_factors, int nelr) {
    // const int i = (blockDim.x*blockIdx.x + threadIdx.x);
    const int i = get_global_id(0);
    if (i >= nelr)
        return;
    ...
    // it all at once
    // step_factors[i] = (float)(0.5f) / (sqrtf(areas[i]) * (sqrtf(speed_sqd) +
    // speed_of_sound));
    step_factors[i] =
        (float)(0.5f) / (sqrt(areas[i]) * (sqrt(speed_sqd) + speed_of_sound));
}

__kernel void
compute_flux(__global int *elements_surrounding_elements,
             __global float *normals, __global float *variables,
             __constant float *ff_variable, __global float *fluxes,
             ...
             __constant FLOAT3 *ff_flux_contribution_momentum_z, int nelr) {
...
__kernel void time_step(int j, int nelr, __global float *old_variables,
                        __global float *variables, __global float *step_factors,
