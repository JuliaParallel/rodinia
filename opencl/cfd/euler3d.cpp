/********************************************************************
        ...
        - original code from the AIAA-2009-4001 by Andrew Corrigan,
acorriga@gmu.edu
 ...
 * Generic functions
 */
template <typename T> cl_mem alloc(int N) {
    cl_mem mem_d = _clMalloc(sizeof(T) * N);
    return mem_d;
}

template <typename T> void dealloc(cl_mem array) { _clFree(array); }

template <typename T> void copy(cl_mem dst, cl_mem src, int N) {
    _clMemcpyD2D(dst, src, N * sizeof(T));
}

template <typename T> void upload(cl_mem dst, T *src, int N) {
    _clMemcpyH2D(dst, src, N * sizeof(T));
}

template <typename T> void download(T *dst, cl_mem src, int N) {
...
void compute_step_factor(int nelr, cl_mem variables, cl_mem areas,
                         cl_mem step_factors) {
...
void compute_flux(int nelr, cl_mem elements_surrounding_elements,
                  cl_mem normals, cl_mem variables, cl_mem ff_variable,
...
void time_step(int j, int nelr, cl_mem old_variables, cl_mem variables,
               cl_mem step_factors, cl_mem fluxes) {
...
int main(int argc, char **argv) {
    ...
    if (argc < 2) {
        std::cout << "specify data file name and [device type] [device id]"
                  << std::endl;
        return 0;
    }
    const char *data_file_name = argv[1];
    _clCmdParams(argc, argv);
    cl_mem ff_variable, ff_flux_contribution_momentum_x,
    ...
    try {
        ...
        // set far field conditions and load them into constant memory on the
        // gpu
        {
            // float h_ff_variable[NVAR];
            ...
            _clMemcpyH2D(ff_flux_contribution_momentum_x,
                         &h_ff_flux_contribution_momentum_x, sizeof(float3));
            _clMemcpyH2D(ff_flux_contribution_momentum_y,
                         &h_ff_flux_contribution_momentum_y, sizeof(float3));
            _clMemcpyH2D(ff_flux_contribution_momentum_z,
                         &h_ff_flux_contribution_momentum_z, sizeof(float3));
            _clMemcpyH2D(ff_flux_contribution_density_energy,
                         &h_ff_flux_contribution_density_energy,
                         sizeof(float3));
            _clFinish();
        }
        ...
        // float* normals;
        {
            ...
            std::cout << "--cambine: nel=" << nel << ", nelr=" << nelr
                      << std::endl;
            ...
            for (int i = 0; i < nel; i++) {
                ...
                for (int j = 0; j < NNB; j++) {
                    ...
                    h_elements_surrounding_elements[i +
                                                    j * nelr]--; // it's coming
                                                                 // in with
                                                                 // Fortran
                                                                 // numbering
        ...
        _clFinish();
        // these need to be computed the first time in order to compute time
        // step
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            for (int j = 0; j < RK; j++) {
                ...
                time_step(j, nelr, old_variables, variables, step_factors,
                          fluxes);
            }
        }
        _clFinish();
        ...
        std::cout << "--cambine:( an exception catched in main body ->" << msg
                  << std::endl;
        _clFree(ff_variable);
        _clFree(ff_flux_contribution_momentum_x);
        _clFree(ff_flux_contribution_momentum_y);
        ...
        std::cout << "--cambine:( unknow exceptions in main body..."
                  << std::endl;
        _clFree(ff_variable);
        _clFree(ff_flux_contribution_momentum_x);
        _clFree(ff_flux_contribution_momentum_y);
