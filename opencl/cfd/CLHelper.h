...
int total_kernels = 5;
// string kernel_names[9] = {"memset_kernel", "initialize_variables",
// "compute_step_factor", "compute_flux", "time_step", "compute_speed_sqd",
// "compute_velocity", "compute_pressure", "compute_speed_of_sound"};
string kernel_names[5] = {"memset_kernel", "initialize_variables",
...
void _clSetDevice(int idx) throw(string) {
    ...
    if (resultCL != CL_SUCCESS)
        throw(string(
            "InitCL()::Error: Getting number of platforms (clGetPlatformIDs)"));
    ...
    if (resultCL != CL_SUCCESS)
        throw(
            string("InitCL()::Error: Getting platform ids (clGetPlatformIDs)"));

    /* Select the target platform. Default: first platform */
    targetPlatform = allPlatforms[0];
    free(allPlatforms);

    oclHandles.cl_status = clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_ALL, 0,
                                          NULL, &deviceListSize);
    ...
    for (int i = 0; i < argc; ++i) {
        ...
        case 't': //--t stands for device type
            if (++i < argc) {
                ...
                std::cerr << "Could not read argument after option "
                          << argv[i - 1] << std::endl;
                throw;
            }
            break;
...
/*------------------------------------------------------------
        ...
        @params:
                device_id: device id
                device_type: the types of devices, e.g. CPU, GPU,
ACCERLERATOR,...
        ...
        @revised:
                get the number of devices and devices have no relationship with
context
        @date:		24/03/2011
------------------------------------------------------------*/
...
// printf("number of platforms:%d\n",numPlatforms);	//by cambine
#ifdef DEV_INFO
    std::cout << "--cambine: number of platforms: " << numPlatforms
              << std::endl;
#endif
    ...
    for (int i = 0; i < numPlatforms; i++) {
        char pbuff[128];
        resultCL = clGetPlatformInfo(allPlatforms[i], CL_PLATFORM_VENDOR,
                                     sizeof(pbuff), pbuff, NULL);
        if (resultCL != CL_SUCCESS)
            throw(string(
                "InitCL()::Error: Getting platform info (clGetPlatformInfo)"));
...
// printf("vedor is %s\n",pbuff);
    ...
    if (device_type.compare("") != 0) {
        if (device_type.compare("cpu") == 0) {
            oclHandles.cl_status = clGetDeviceIDs(
                targetPlatform, CL_DEVICE_TYPE_CPU, 0, NULL, &deviceListSize);
            if (oclHandles.cl_status != CL_SUCCESS) {
        ...
        if (device_type.compare("gpu") == 0) {
            oclHandles.cl_status = clGetDeviceIDs(
                targetPlatform, CL_DEVICE_TYPE_GPU, 0, NULL, &deviceListSize);
            if (oclHandles.cl_status != CL_SUCCESS) {
        ...
        if (device_type.compare("acc") == 0) {
            oclHandles.cl_status =
                clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_ACCELERATOR, 0,
                               NULL, &deviceListSize);
            if (oclHandles.cl_status != CL_SUCCESS) {
                throw(string(
                    "exception in _clInit -> clGetDeviceIDs -> ACCELERATOR"));
            }
        }
    } else {
        oclHandles.cl_status = clGetDeviceIDs(
            targetPlatform, CL_DEVICE_TYPE_ALL, 0, NULL, &deviceListSize);
        if (oclHandles.cl_status != CL_SUCCESS) {
    ...
    oclHandles.devices =
        (cl_device_id *)malloc(sizeof(cl_device_id) * deviceListSize);
    ...
    if (device_type.compare("") != 0) {
        if (device_type.compare("cpu") == 0) {
            oclHandles.cl_status =
                clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_CPU,
                               deviceListSize, oclHandles.devices, NULL);
            if (oclHandles.cl_status != CL_SUCCESS) {
                throw(string(
                    "exception in _clInit -> clGetDeviceIDs -> CPU ->2"));
            }
        }
        if (device_type.compare("gpu") == 0) {
            oclHandles.cl_status =
                clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_GPU,
                               deviceListSize, oclHandles.devices, NULL);
            if (oclHandles.cl_status != CL_SUCCESS) {
                throw(string(
                    "exception in _clInit -> clGetDeviceIDs -> GPU -> 2"));
            }
        }
        if (device_type.compare("acc") == 0) {
            oclHandles.cl_status =
                clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_ACCELERATOR,
                               deviceListSize, oclHandles.devices, NULL);
            if (oclHandles.cl_status != CL_SUCCESS) {
                throw(string("exception in _clInit -> clGetDeviceIDs -> "
                             "ACCELERATOR -> 2"));
        ...
        oclHandles.cl_status =
            clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_ALL, deviceListSize,
                           oclHandles.devices, NULL);
        if (oclHandles.cl_status != CL_SUCCESS) {
...
//--cambine-3: create an OpenCL context
    ...
    oclHandles.context = clCreateContext(0, deviceListSize, oclHandles.devices,
                                         NULL, NULL, &resultCL);

    if ((resultCL != CL_SUCCESS) || (oclHandles.context == NULL))
        throw(string(
            "InitCL()::Error: Creating Context (clCreateContextFromType)"));
#ifdef DEV_INFO
    ...
    if ((resultCL != CL_SUCCESS) || (oclHandles.queue == NULL))
        throw(
            string("InitCL()::Creating Command Queue. (clCreateCommandQueue)"));
#ifdef PROFILE_
    double t2 = gettime();
    CC += t2 - t1;
#endif
    //-----------------------------------------------
    //--cambine-5: Load CL file, build CL program object, create CL kernel
    //object
    ...
    oclHandles.program = clCreateProgramWithSource(
        oclHandles.context, 1, &source, sourceSize, &resultCL);
    ...
    resultCL = clBuildProgram(oclHandles.program, deviceListSize,
                              oclHandles.devices, options.c_str(), NULL, NULL);
...
// get program information in intermediate representation
    ...
    for (int nKernel = 0; nKernel < total_kernels; nKernel++) {
        ...
        if ((resultCL != CL_SUCCESS) || (kernel == NULL)) {
            string errorMsg =
                "InitCL()::Error: Creating Kernel (clCreateKernel) \"" +
...
// get resource alocation information
    ...
    if (oclHandles.cl_status != CL_SUCCESS) {
        throw(string("exceptions in _InitCL -> getting resources allocation "
                     "information-2"));
    }
...
#endif
    cl_mem d_mem;
    d_mem = clCreateBuffer(oclHandles.context, CL_MEM_READ_WRITE, size, NULL,
                           &oclHandles.cl_status);
#ifdef ERRMSG
    if (oclHandles.cl_status != CL_SUCCESS) {
...
void *_clMallocHost(int size) throw(string) {
    ...
    mem_h = clEnqueueMapBuffer(oclHandles.queue, oclHandles.pinned_mem_out,
                               CL_TRUE, CL_MAP_READ, 0, size, 0, NULL, NULL,
                               &oclHandles.cl_status);

#ifdef ERRMSG
    if (oclHandles.cl_status != CL_SUCCESS || mem_h == NULL) {
        oclHandles.error_str =
            "excpetion in _clMallocHost -> clEnqueueMapBuffer";
        switch (oclHandles.cl_status) {
...
void _clFreeHost(int io, void *mem_h) {
    if (io == 0) { // in
        if (mem_h) {
            oclHandles.cl_status = clEnqueueUnmapMemObject(
                oclHandles.queue, oclHandles.pinned_mem_in, (void *)mem_h, 0,
                NULL, NULL);
#ifdef ERRMSG
            if (oclHandles.cl_status != CL_SUCCESS) {
                oclHandles.error_str =
                    "excpetion in _clFreeHost -> clEnqueueUnmapMemObject(in)";
                switch (oclHandles.cl_status) {
                case CL_INVALID_COMMAND_QUEUE:
                    oclHandles.error_str += "CL_INVALID_COMMAND_QUEUE";
                    break;
        ...
        if (mem_h) {
            oclHandles.cl_status = clEnqueueUnmapMemObject(
                oclHandles.queue, oclHandles.pinned_mem_out, (void *)mem_h, 0,
                NULL, NULL);
#ifdef ERRMSG
            if (oclHandles.cl_status != CL_SUCCESS) {
                oclHandles.error_str =
...
void _clMemcpyH2D(cl_mem dst, const void *src, int size) throw(string) {
    ...
    double t1 = gettime();
#endif
    oclHandles.cl_status = clEnqueueWriteBuffer(oclHandles.queue, dst, CL_TRUE,
...
void _clMemcpyD2H(void *dst, cl_mem src, int size) throw(string) {
    ...
    double t1 = gettime();
#endif
    oclHandles.cl_status = clEnqueueReadBuffer(oclHandles.queue, src, CL_TRUE,
                                               0, size, dst, 0, 0, 0);
#ifdef ERRMSG
    if (oclHandles.cl_status != CL_SUCCESS) {
...
void _clMemcpyD2D(cl_mem dst, cl_mem src, int size) throw(string) {
    ...
    double t1 = gettime();
#endif
    oclHandles.cl_status = clEnqueueCopyBuffer(oclHandles.queue, src, dst, 0, 0,
                                               size, 0, NULL, NULL);
#ifdef ERRMSG
    if (oclHandles.cl_status != CL_SUCCESS) {
...
//	oclHandles.cl_status = clWaitForEvents(1, &e[0]);
    ...
    if (oclHandles.cl_status != CL_SUCCESS) {
        oclHandles.error_str =
            "excpetion in _clEnqueueNDRange() -> clWaitForEvents ->";
        switch (oclHandles.cl_status) {
        case CL_INVALID_VALUE:
            oclHandles.error_str += "CL_INVALID_VALUE";
            break;
...
/*------------------------------------------------------------
        ...
        @params:
                ...
                number_elements:	the number of elements in the selected
memory
        @return:	NULL
        @date:		03/04/2011
------------------------------------------------------------*/
...
/*------------------------------------------------------------
        @function:	entry of invoke the kernel function using 2d working
items
        @params:
                kernel_id:	the index of kernel to set
                range_x:	the number of working items in x direction
                range_y:	the number of working items in y direction
                group_x:	the number of working items in each work group
in x direction
                group_y:	the number of working items	in each work
group in y direction
        @return:	NULL
        @date:		03/04/2011
------------------------------------------------------------*/
