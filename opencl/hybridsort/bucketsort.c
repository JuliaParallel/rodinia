...
void init_bucketsort(int listsize) {
    ...
    //    CL_DEVICE_TYPE_CPU, 2, &device_id, NULL);
    ...
    bucketCommands = clCreateCommandQueue(bucketContext, devices[0],
    ...
    d_offsets_buff =
        clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                       DIVISIONS * sizeof(unsigned int), NULL, NULL);
    pivotPoints = (float *)malloc(DIVISIONS * sizeof(float));

    d_indice_buff = clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                                   listsize * sizeof(int), NULL, NULL);
    d_indice_input_buff = clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                                         listsize * sizeof(int), NULL, NULL);
    ...
    l_pivotpoints_buff = clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                                        DIVISIONS * sizeof(float), NULL, NULL);
    l_offsets_buff =
        clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                       DIVISIONS * sizeof(unsigned int), NULL, NULL);

    int blocks = ((listsize - 1) / (BUCKET_THREAD_N * BUCKET_BAND)) + 1;
    d_prefixoffsets_buff =
        clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                       blocks * BUCKET_BLOCK_MEMORY * sizeof(int), NULL, NULL);
    d_prefixoffsets =
        (unsigned int *)malloc(blocks * BUCKET_BLOCK_MEMORY * sizeof(int));
    d_prefixoffsets_altered =
        (unsigned int *)malloc(blocks * BUCKET_BLOCK_MEMORY * sizeof(int));
    d_prefixoffsets_input_buff =
        clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                       blocks * BUCKET_BLOCK_MEMORY * sizeof(int), NULL, NULL);
    bucketOutput = clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                                  (listsize + (DIVISIONS * 4)) * sizeof(float),
                                  NULL, NULL);
    FILE *fp;
    ...
    if (err != CL_SUCCESS) {
        ...
        clGetProgramBuildInfo(bucketProgram, devices[0], CL_PROGRAM_BUILD_LOG,
...
void histogramInit(int listsize) {
    ...
    cl_context_properties contextProperties[] = {
        CL_CONTEXT_PLATFORM, (cl_context_properties)platformID[num], 0};

    histoContext =
        clCreateContext(contextProperties, 1, &devices[0], NULL, NULL, &err);

    histoCommands = clCreateCommandQueue(histoContext, devices[0],
                                         CL_QUEUE_PROFILING_ENABLE, &err);
    histoInput = clCreateBuffer(histoContext, CL_MEM_READ_ONLY,
                                listsize * (sizeof(float)), NULL, NULL);
    histoOutput = clCreateBuffer(histoContext, CL_MEM_READ_WRITE,
                                 1024 * sizeof(unsigned int), NULL, NULL);
    FILE *fp;
    ...
    if (err != CL_SUCCESS) {
        ...
        clGetProgramBuildInfo(histoProgram, devices[0], CL_PROGRAM_BUILD_LOG,
                              sizeof(buffer), buffer, &len);
        printf("%s\n", buffer);
        exit(1);
    }
...
void histogram1024GPU(unsigned int *h_Result, float *d_Data, float minimum,
                      float maximum, int listsize) {
    err = clEnqueueWriteBuffer(histoCommands, histoInput, CL_TRUE, 0,
                               listsize * sizeof(float), d_Data, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to source array!\n");
        exit(1);
    }
    err = clEnqueueWriteBuffer(histoCommands, histoOutput, CL_TRUE, 0,
                               DIVISIONS * sizeof(unsigned int), h_Result, 0,
                               NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to source array!\n");
        exit(1);
    ...
    local = 96;
#endif
    err = clEnqueueNDRangeKernel(histoCommands, histoKernel, 1, NULL, &global,
                                 &local, 0, NULL, &histoEvent);
    if (err) {
    ...
    err = clEnqueueReadBuffer(histoCommands, histoOutput, CL_TRUE, 0,
                              1024 * sizeof(unsigned int), h_Result, 0, NULL,
                              NULL);
    if (err != CL_SUCCESS) {
    ...
    double total_time;
    clGetEventProfilingInfo(histoEvent, CL_PROFILING_COMMAND_START,
                            sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(histoEvent, CL_PROFILING_COMMAND_END,
...
void bucketSort(float *d_input, float *d_output, int listsize, int *sizes,
    ...
    err = clEnqueueWriteBuffer(bucketCommands, l_pivotpoints_buff, CL_TRUE, 0,
                               DIVISIONS * sizeof(float), pivotPoints, 0, NULL,
                               NULL);
    if (err != CL_SUCCESS) {
    ...
    d_input_buff = clCreateBuffer(bucketContext, CL_MEM_READ_WRITE,
                                  (listsize + (DIVISIONS * 4)) * sizeof(float),
                                  NULL, NULL);

    err = clEnqueueWriteBuffer(bucketCommands, d_input_buff, CL_TRUE, 0,
                               (listsize + (DIVISIONS * 4)) * sizeof(float),
                               d_input, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clSetKernelArg(bucketcountKernel, 2, sizeof(cl_mem),
                         &d_prefixoffsets_buff);
    err = clSetKernelArg(bucketcountKernel, 3, sizeof(cl_int), &listsize);
    err = clSetKernelArg(bucketcountKernel, 4, sizeof(cl_mem),
                         &l_pivotpoints_buff);
    ...
    err = clEnqueueNDRangeKernel(bucketCommands, bucketcountKernel, 3, NULL,
                                 global, local, 0, NULL, &bucketCountEvent);
    if (err) {
    ...
    err =
        clEnqueueReadBuffer(bucketCommands, d_prefixoffsets_buff, CL_TRUE, 0,
                            blocks * BUCKET_BLOCK_MEMORY * sizeof(unsigned int),
                            d_prefixoffsets, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to read prefix output array! %d\n", err);
        exit(1);
    }
    err = clEnqueueReadBuffer(bucketCommands, d_indice_buff, CL_TRUE, 0,
                              listsize * sizeof(int), d_indice, 0, NULL, NULL);
    ...
    cl_ulong time_start, time_end;
    double total_time;
    clGetEventProfilingInfo(bucketCountEvent, CL_PROFILING_COMMAND_START,
                            sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(bucketCountEvent, CL_PROFILING_COMMAND_END,
                            sizeof(time_end), &time_end, NULL);
    total_time = time_end - time_start;
...
//	///////////////////////////////////////////////////////////////////////////
    ...
    bucketprefixKernel =
        clCreateKernel(bucketProgram, "bucketprefixoffset", &err);
    ...
    err = clEnqueueWriteBuffer(bucketCommands, d_prefixoffsets_buff, CL_TRUE, 0,
    ...
    err = 0;
    err = clSetKernelArg(bucketprefixKernel, 0, sizeof(cl_mem),
                         &d_prefixoffsets_buff);
    err =
        clSetKernelArg(bucketprefixKernel, 1, sizeof(cl_mem), &d_offsets_buff);
    err = clSetKernelArg(bucketprefixKernel, 2, sizeof(cl_int), &blocks);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to set kernel arguments! %d\n", err);
        exit(1);
    }
    err = clEnqueueNDRangeKernel(bucketCommands, bucketprefixKernel, 3, NULL,
                                 globalpre, localpre, 0, NULL,
                                 &bucketPrefixEvent);
    if (err) {
    ...
    clFinish(bucketCommands);
    err = clEnqueueReadBuffer(bucketCommands, d_offsets_buff, CL_TRUE, 0,
                              DIVISIONS * sizeof(unsigned int), h_offsets, 0,
                              NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to read d_offsets output array! %d\n", err);
        exit(1);
    }
    err = clEnqueueReadBuffer(bucketCommands, d_prefixoffsets_buff, CL_TRUE, 0,
                              blocks * BUCKET_BLOCK_MEMORY * sizeof(int),
                              d_prefixoffsets_altered, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to read d_offsets output array! %d\n", err);
        exit(1);
    }
    clFinish(bucketCommands);
    clGetEventProfilingInfo(bucketPrefixEvent, CL_PROFILING_COMMAND_START,
                            sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(bucketPrefixEvent, CL_PROFILING_COMMAND_END,
                            sizeof(time_end), &time_end, NULL);
    total_time = time_end - time_start;
    sum += total_time / 1000000;
    ...
    err = clEnqueueWriteBuffer(bucketCommands, l_offsets_buff, CL_TRUE, 0,
                               DIVISIONS * sizeof(unsigned int), h_offsets, 0,
                               NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(bucketCommands, d_indice_input_buff, CL_TRUE, 0,
                               listsize * sizeof(int), d_indice, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to d_input_buff source array!\n");
        exit(1);
    }
    err = clEnqueueWriteBuffer(bucketCommands, d_prefixoffsets_input_buff,
                               CL_TRUE, 0,
                               blocks * BUCKET_BLOCK_MEMORY * sizeof(int),
                               d_prefixoffsets_altered, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    err = clEnqueueWriteBuffer(bucketCommands, bucketOutput, CL_TRUE, 0,
                               (listsize + (DIVISIONS * 4)) * sizeof(float),
                               d_output, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to source array!\n");
        exit(1);
    }
    ...
    err = clSetKernelArg(bucketsortKernel, 4, sizeof(cl_mem),
                         &d_prefixoffsets_input_buff);
    ...
    err = clEnqueueReadBuffer(bucketCommands, bucketOutput, CL_TRUE, 0,
                              (listsize + (DIVISIONS * 4)) * sizeof(float),
                              d_output, 0, NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    clFinish(bucketCommands);
    clGetEventProfilingInfo(bucketSortEvent, CL_PROFILING_COMMAND_START,
                            sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(bucketSortEvent, CL_PROFILING_COMMAND_END,
                            sizeof(time_end), &time_end, NULL);
    total_time = time_end - time_start;
    sum += total_time / 1000000;
    printf("Bucket Sort Kernel Time: %0.3f \n", total_time / 1000000);
}
double getBucketTime() { return sum; }
////////////////////////////////////////////////////////////////////////////////
