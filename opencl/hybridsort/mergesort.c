...
void init_mergesort(int listsize) {
    ...
    mergeCommands = clCreateCommandQueue(mergeContext, devices[0],
                                         CL_QUEUE_PROFILING_ENABLE, &err);

    d_resultList_first_altered = (cl_float4 *)malloc(listsize * sizeof(float));
    d_resultList_first_buff = clCreateBuffer(
        mergeContext, CL_MEM_READ_WRITE, listsize * sizeof(float), NULL, NULL);
    d_origList_first_buff = clCreateBuffer(
        mergeContext, CL_MEM_READ_WRITE, listsize * sizeof(float), NULL, NULL);

    d_orig = clCreateBuffer(mergeContext, CL_MEM_READ_WRITE,
                            listsize * sizeof(float), NULL, NULL);
    d_res = clCreateBuffer(mergeContext, CL_MEM_READ_WRITE,
                           listsize * sizeof(float), NULL, NULL);
    ...
    if (err != CL_SUCCESS) {
        ...
        clGetProgramBuildInfo(mergeProgram, device_id, CL_PROGRAM_BUILD_LOG,
...
cl_float4 *runMergeSort(int listsize, int divisions, cl_float4 *d_origList,
                        cl_float4 *d_resultList, int *sizes, int *nullElements,
    ...
    err = clEnqueueWriteBuffer(mergeCommands, d_resultList_first_buff, CL_TRUE,
                               0, listsize * sizeof(float), d_resultList, 0,
                               NULL, NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to d_resultList_first_buff source "
               "array!\n");
        exit(1);
    }
    err = clEnqueueWriteBuffer(mergeCommands, d_origList_first_buff, CL_TRUE, 0,
                               listsize * sizeof(float), d_origList, 0, NULL,
                               NULL);
    if (err != CL_SUCCESS) {
        printf(
            "Error: Failed to write to d_origList_first_buff source array!\n");
        exit(1);
    }

    err = 0;
    err = clSetKernelArg(mergeFirstKernel, 0, sizeof(cl_mem),
                         &d_origList_first_buff);
    err = clSetKernelArg(mergeFirstKernel, 1, sizeof(cl_mem),
                         &d_resultList_first_buff);
    ...
    err = clEnqueueNDRangeKernel(mergeCommands, mergeFirstKernel, 3, NULL,
                                 global, local, 0, NULL, &mergeFirstEvent);
    if (err) {
    ...
    err = clEnqueueReadBuffer(mergeCommands, d_resultList_first_buff, CL_TRUE,
                              0, listsize * sizeof(float), d_resultList, 0,
                              NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    double total_time;
    clGetEventProfilingInfo(mergeFirstEvent, CL_PROFILING_COMMAND_START,
                            sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(mergeFirstEvent, CL_PROFILING_COMMAND_END,
    ...
    //        printf("TEST %f \n", d_resultList[i].s[3]);
    //    }
    constStartAddr = clCreateBuffer(mergeContext, CL_MEM_READ_WRITE,
                                    (divisions + 1) * sizeof(int), NULL, NULL);

    err = clEnqueueWriteBuffer(mergeCommands, constStartAddr, CL_TRUE, 0,
                               (divisions + 1) * sizeof(int), startaddr, 0,
                               NULL, NULL);
    if (err != CL_SUCCESS) {
...
//        printf("TN %d \n", threadsNeeded);
        ...
        err = clEnqueueWriteBuffer(mergeCommands, d_resultList_first_buff,
                                   CL_TRUE, 0, listsize * sizeof(float),
                                   d_resultList, 0, NULL, NULL);
        if (err != CL_SUCCESS) {
            printf("Error: Failed to write to d_resultList_first_buff source "
                   "array!\n");
            exit(1);
        ...
        err = clEnqueueWriteBuffer(mergeCommands, d_origList_first_buff,
                                   CL_TRUE, 0, listsize * sizeof(float),
                                   d_origList, 0, NULL, NULL);
        if (err != CL_SUCCESS) {
            printf("Error: Failed to write to d_origList_first_buff source "
                   "array!\n");
            exit(1);
        }
        err = 0;
        err = clSetKernelArg(mergePassKernel, 0, sizeof(cl_mem),
                             &d_origList_first_buff);
        err = clSetKernelArg(mergePassKernel, 1, sizeof(cl_mem),
                             &d_resultList_first_buff);
        err = clSetKernelArg(mergePassKernel, 2, sizeof(cl_int), &nrElems);
        err = clSetKernelArg(mergePassKernel, 3, sizeof(int), &threadsPerDiv);
        err =
            clSetKernelArg(mergePassKernel, 4, sizeof(cl_mem), &constStartAddr);
        if (err != CL_SUCCESS) {
        ...
        err = clEnqueueNDRangeKernel(mergeCommands, mergePassKernel, 3, NULL,
                                     global, local, 0, NULL, &mergePassEvent);
        if (err) {
        ...
        clFinish(mergeCommands);
        err = clEnqueueReadBuffer(mergeCommands, d_resultList_first_buff,
                                  CL_TRUE, 0, listsize * sizeof(float),
                                  d_resultList, 0, NULL, NULL);
        if (err != CL_SUCCESS) {
            printf("Error: Failed to read prefix output array! %d\n", err);
        ...
        clFinish(mergeCommands);
        clGetEventProfilingInfo(mergePassEvent, CL_PROFILING_COMMAND_START,
                                sizeof(time_start), &time_start, NULL);
        clGetEventProfilingInfo(mergePassEvent, CL_PROFILING_COMMAND_END,
                                sizeof(time_end), &time_end, NULL);
        total_time = time_end - time_start;
        mergesum += total_time / 1000000;
    ...
    finalStartAddr = clCreateBuffer(mergeContext, CL_MEM_READ_WRITE,
                                    (divisions + 1) * sizeof(int), NULL, NULL);

    err = clEnqueueWriteBuffer(mergeCommands, finalStartAddr, CL_TRUE, 0,
                               (divisions + 1) * sizeof(int), origOffsets, 0,
                               NULL, NULL);
    if (err != CL_SUCCESS) {
    ...
    nullElems = clCreateBuffer(mergeContext, CL_MEM_READ_WRITE,
                               (divisions) * sizeof(int), NULL, NULL);

    err = clEnqueueWriteBuffer(mergeCommands, nullElems, CL_TRUE, 0,
                               (divisions) * sizeof(int), nullElements, 0, NULL,
                               NULL);
    if (err != CL_SUCCESS) {
    ...
    local[0] = 256;
#endif
    grid[0] = ((largestSize % local[0]) == 0) ? largestSize / local[0]
    ...
    err = clEnqueueWriteBuffer(mergeCommands, d_res, CL_TRUE, 0,
                               listsize * sizeof(float), d_resultList, 0, NULL,
                               NULL);
    if (err != CL_SUCCESS) {
        printf("Error: Failed to write to d_resultList_first_buff source "
               "array!\n");
    ...
    err = clEnqueueWriteBuffer(mergeCommands, d_orig, CL_TRUE, 0,
                               listsize * sizeof(float), d_origList, 0, NULL,
                               NULL);
    if (err != CL_SUCCESS) {
        printf(
    ...
    err = clEnqueueNDRangeKernel(mergeCommands, mergePackKernel, 3, NULL,
                                 global, local, 0, NULL, &mergePackEvent);
    if (err) {
        printf("Error: Failed to execute merge pack kernel!\n");
        exit(1);
    }
    clFinish(mergeCommands);
    clGetEventProfilingInfo(mergePackEvent, CL_PROFILING_COMMAND_START,
                            sizeof(time_start), &time_start, NULL);
    clGetEventProfilingInfo(mergePackEvent, CL_PROFILING_COMMAND_END,
                            sizeof(time_end), &time_end, NULL);
    total_time = time_end - time_start;
    mergesum += total_time / 1000000;
    printf("Merge Pack Kernel Time: %0.3f \n", total_time / 1000000);
    err = clEnqueueReadBuffer(mergeCommands, d_orig, CL_TRUE, 0,
                              listsize * sizeof(float), d_origList, 0, NULL,
                              NULL);
    if (err != CL_SUCCESS) {
...
double getMergeTime() { return mergesum; }
