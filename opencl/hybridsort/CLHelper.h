...
void _clInit() {
    ...
    oclHandles.context = clCreateContextFromType(cprops, CL_DEVICE_TYPE_GPU,
                                                 NULL, NULL, &resultCL);
    ...
    oclHandles.cl_status =
        clGetDeviceIDs(targetPlatform, CL_DEVICE_TYPE_GPU, deviceListSize,
                       oclHandles.devices, NULL);
    if (oclHandles.cl_status != CL_SUCCESS) {
...
cl_mem _clCreateAndCpyPinnedMem(int size, float *h_mem_source) throw(string) {
    ...
    if (oclHandles.cl_status != CL_SUCCESS)
        throw(
            string("excpetion in _clCreateAndCpyMem() -> clEnqueueMapBuffer"));
#endif
...
#pragma omp parallel for
    ...
    if (oclHandles.cl_status != CL_SUCCESS)
        throw(string(
            "excpetion in _clCreateAndCpyMem() -> clEnqueueWriteBuffer"));
#endif
