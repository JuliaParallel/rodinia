...
inline void addData1024(volatile __local uint *s_WarpHist, uint data,
                        uint tag) {
    uint count;
    do {
...
__kernel void histogram1024Kernel(__global uint *d_Result,
                                  __global float *d_Data, float minimum,
                                  float maximum, uint dataCount) {
    ...
    for (int pos = get_global_id(0); pos < dataCount;
