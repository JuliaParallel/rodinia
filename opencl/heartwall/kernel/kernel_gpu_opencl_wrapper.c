...
#include "./../heartwall.h" // (in main directory)						needed to recognized input parameters
...
void kernel_gpu_opencl_wrapper(params_common common, int *endoRow, int *endoCol,
    ...
    error = clGetPlatformIDs(0, NULL, &num_platforms); // # of platforms
    if (error != CL_SUCCESS)
    ...
    for (plat_count = 0; plat_count < num_platforms; plat_count++) {
        ...
        error = clGetPlatformInfo(platform, CL_PLATFORM_VENDOR, sizeof(pbuf),
                                  pbuf, NULL);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);
        printf("\tPlatform %d: %s\n", plat_count, pbuf);
    ...
    error = clGetContextInfo(
        context, CL_CONTEXT_DEVICES, 0, NULL,
    ...
    for (devi_count = 0; devi_count < num_devices; devi_count++) {
        ...
        error =
            clGetDeviceInfo(device, CL_DEVICE_NAME, sizeof(dbuf), dbuf, NULL);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);
        printf("\tDevice %d: %s\n", devi_count, dbuf);
    }

    // Select device (previousely selected for the context) (if there are
    // multiple devices, choose the first one)
    int devi = 0;
    device = devices[devi];
    ...
    cl_mem d_common;
    d_common = clCreateBuffer(context, CL_MEM_READ_WRITE, common.common_mem,
                              NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // common
    cl_mem d_frame;
    d_frame = clCreateBuffer(context, CL_MEM_READ_WRITE, common.frame_mem, NULL,
                             &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // common
    cl_mem d_endoRow;
    d_endoRow = clCreateBuffer(context, CL_MEM_READ_WRITE, common.endo_mem,
                               NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    cl_mem d_endoCol;
    d_endoCol = clCreateBuffer(context, CL_MEM_READ_WRITE, common.endo_mem,
                               NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_tEndoRowLoc =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       common.endo_mem * common.no_frames, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // common
    cl_mem d_epiRow;
    d_epiRow = clCreateBuffer(context, CL_MEM_READ_WRITE, common.epi_mem, NULL,
                              &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    cl_mem d_epiCol;
    d_epiCol = clCreateBuffer(context, CL_MEM_READ_WRITE, common.epi_mem, NULL,
                              &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_tEpiRowLoc =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       common.epi_mem * common.no_frames, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // common
    cl_mem d_endoT;
    d_endoT = clCreateBuffer(context, CL_MEM_READ_WRITE,
                             common.in_mem * common.endoPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // common
    cl_mem d_epiT;
    d_epiT = clCreateBuffer(context, CL_MEM_READ_WRITE,
                            common.in_mem * common.epiPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // unique
    cl_mem d_in2;
    d_in2 = clCreateBuffer(context, CL_MEM_READ_WRITE,
                           common.in2_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // unique
    cl_mem d_conv;
    d_conv = clCreateBuffer(context, CL_MEM_READ_WRITE,
                            common.conv_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_in2_pad_cumv = clCreateBuffer(context, CL_MEM_READ_WRITE,
    ...
    d_in2_pad_cumv_sel = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        common.in2_pad_cumv_sel_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_in2_sub_cumh = clCreateBuffer(context, CL_MEM_READ_WRITE,
    ...
    d_in2_sub_cumh_sel = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        common.in2_sub_cumh_sel_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_in2_sub2 =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       common.in2_sub2_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_in2_sqr =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       common.in2_sqr_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_in2_sqr_sub2 = clCreateBuffer(context, CL_MEM_READ_WRITE,
    ...
    d_in_sqr =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       common.in_sqr_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // unique
    cl_mem d_tMask;
    d_tMask = clCreateBuffer(context, CL_MEM_READ_WRITE,
                             common.tMask_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    d_mask_conv =
        clCreateBuffer(context, CL_MEM_READ_WRITE,
                       common.mask_conv_mem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_endoRow, 1, 0,
                                 common.endo_mem, endoRow, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    error = clEnqueueWriteBuffer(command_queue, d_endoCol, 1, 0,
                                 common.endo_mem, endoCol, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueWriteBuffer(command_queue, d_epiRow, 1, 0, common.epi_mem,
                                 epiRow, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    error = clEnqueueWriteBuffer(command_queue, d_epiCol, 1, 0, common.epi_mem,
                                 epiCol, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    printf("# of workgroups = %d, # of threads/workgroup = %d (ensure that "
           "device can handle)\n",
    ...
    error = clEnqueueWriteBuffer(command_queue, d_common, 1, 0,
                                 common.common_mem, &common, 0, 0, 0);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // structures
    error = clSetKernelArg(kernel, 0, sizeof(params_common), (void *)&common);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    // common
    error = clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *)&d_endoRow);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 4, sizeof(cl_mem), (void *)&d_endoCol);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 5, sizeof(cl_mem), (void *)&d_tEndoRowLoc);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 6, sizeof(cl_mem), (void *)&d_tEndoColLoc);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 7, sizeof(cl_mem), (void *)&d_epiRow);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 8, sizeof(cl_mem), (void *)&d_epiCol);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 9, sizeof(cl_mem), (void *)&d_tEpiRowLoc);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 10, sizeof(cl_mem), (void *)&d_tEpiColLoc);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    // common_unique
    error = clSetKernelArg(kernel, 11, sizeof(cl_mem), (void *)&d_endoT);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 12, sizeof(cl_mem), (void *)&d_epiT);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 13, sizeof(cl_mem), (void *)&d_in2);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 14, sizeof(cl_mem), (void *)&d_conv);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 15, sizeof(cl_mem), (void *)&d_in2_pad_cumv);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error =
        clSetKernelArg(kernel, 16, sizeof(cl_mem), (void *)&d_in2_pad_cumv_sel);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 17, sizeof(cl_mem), (void *)&d_in2_sub_cumh);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error =
        clSetKernelArg(kernel, 18, sizeof(cl_mem), (void *)&d_in2_sub_cumh_sel);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 19, sizeof(cl_mem), (void *)&d_in2_sub2);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 20, sizeof(cl_mem), (void *)&d_in2_sqr);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 21, sizeof(cl_mem), (void *)&d_in2_sqr_sub2);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 22, sizeof(cl_mem), (void *)&d_in_sqr);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 23, sizeof(cl_mem), (void *)&d_tMask);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 24, sizeof(cl_mem), (void *)&d_mask_conv);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    // int local_size;
    // local_size = (common.in_elem + common.in_cols + common.in_sqr_rows +
    // common.mask_conv_rows) * 4 + common.mask_conv_rows * 2;
    // printf("size of used local memory/workgroup = %dB (ensure that device can
    // handle)\n", local_size);

    cl_mem d_in_mod_temp;
    d_in_mod_temp = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        sizeof(fp) * common.in_elem * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    cl_mem in_partial_sum;
    in_partial_sum = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        sizeof(fp) * common.in_cols * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    cl_mem in_sqr_partial_sum;
    in_sqr_partial_sum = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        sizeof(fp) * common.in_sqr_rows * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    cl_mem par_max_val;
    par_max_val = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        sizeof(fp) * common.mask_conv_rows * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    cl_mem par_max_coo;
    par_max_coo = clCreateBuffer(
        context, CL_MEM_READ_WRITE,
        sizeof(int) * common.mask_conv_rows * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    cl_mem in_final_sum;
    in_final_sum = clCreateBuffer(context, CL_MEM_READ_WRITE,
                                  sizeof(fp) * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    cl_mem denomT;
    denomT = clCreateBuffer(context, CL_MEM_READ_WRITE,
                            sizeof(fp) * common.allPoints, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);


    error = clSetKernelArg(kernel, 25, sizeof(cl_mem), (void *)&d_in_mod_temp);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 26, sizeof(cl_mem), (void *)&in_partial_sum);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error =
        clSetKernelArg(kernel, 27, sizeof(cl_mem), (void *)&in_sqr_partial_sum);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 28, sizeof(cl_mem), (void *)&par_max_val);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 29, sizeof(cl_mem), (void *)&par_max_coo);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 30, sizeof(cl_mem), (void *)&in_final_sum);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error =
        clSetKernelArg(kernel, 31, sizeof(cl_mem), (void *)&in_sqr_final_sum);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    error = clSetKernelArg(kernel, 32, sizeof(cl_mem), (void *)&denomT);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);


    cl_mem d_checksum;
    d_checksum = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(fp) * CHECK,
                                NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);

    error = clSetKernelArg(kernel, 33, sizeof(cl_mem), (void *)&d_checksum);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    for (frame_no = 0; frame_no < common.frames_processed; frame_no++) {
        ...
        error = clEnqueueWriteBuffer(command_queue, d_frame, 1, 0,
                                     common.frame_mem, frame, 0, 0, 0);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);
        ...
        // common_change
        error = clSetKernelArg(kernel, 1, sizeof(cl_mem), (void *)&d_frame);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);
        error = clSetKernelArg(kernel, 2, sizeof(int), (void *)&frame_no);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);
        ...
        // Synchronization, wait for all operations in the command queue so far
        // to finish
...
//==================================================50
        ...
        error =
            clEnqueueReadBuffer(command_queue, d_checksum, CL_TRUE, 0,
                                sizeof(fp) * CHECK, checksum, 0, NULL, NULL);
        if (error != CL_SUCCESS)
            fatal_CL(error, __LINE__);
        printf("CHECKSUM:\n");
        for (i = 0; i < CHECK; i++) {
    ...
    error = clEnqueueReadBuffer(command_queue, d_tEndoRowLoc, CL_TRUE, 0,
                                common.endo_mem * common.no_frames, tEndoRowLoc,
                                0, NULL, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
...
// for testing of the output
    ...
    error = clEnqueueReadBuffer(command_queue, d_tEndoColLoc, CL_TRUE, 0,
                                common.endo_mem * common.no_frames, tEndoColLoc,
                                0, NULL, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    error = clEnqueueReadBuffer(command_queue, d_tEpiRowLoc, CL_TRUE, 0,
                                common.epi_mem * common.no_frames, tEpiRowLoc,
                                0, NULL, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ;

    error = clEnqueueReadBuffer(command_queue, d_tEpiColLoc, CL_TRUE, 0,
                                common.epi_mem * common.no_frames, tEpiColLoc,
                                0, NULL, NULL);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
