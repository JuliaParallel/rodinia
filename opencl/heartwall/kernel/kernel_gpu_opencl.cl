...
__kernel void kernel_gpu_opencl( // structures
    ...
    __global int
        *d_tEndoRowLoc, // 5	OUTPUT	common.endoPoints * common.no_frames
    __global int
        *d_tEndoColLoc,     // 6	OUTPUT	common.endoPoints * common.no_frames
    __global int *d_epiRow, // 7	INPUT
    __global int *d_epiCol, // 8	INPUT
    __global int
        *d_tEpiRowLoc, // 9	OUTPUT	common.epiPoints * common.no_frames
    __global int
        *d_tEpiColLoc, // 10	OUTPUT	common.epiPoints * common.no_frames
    ...
    __global fp
        *d_in2_all, // 13	OUTPUT	common.in2_elem * common.allPoints
    __global fp
        *d_conv_all, // 14	OUTPUT	common.conv_elem * common.allPoints
    __global fp *d_in2_pad_cumv_all,     // 15	OUTPUT
                                         // common.in2_pad_cumv_elem *
                                         // common.allPoints
    __global fp *d_in2_pad_cumv_sel_all, // 16	OUTPUT
                                         // common.in2_pad_cumv_sel_elem *
                                         // common.allPoints
    __global fp *d_in2_sub_cumh_all,     // 17	OUTPUT
                                         // common.in2_sub_cumh_elem *
                                         // common.allPoints
    __global fp *d_in2_sub_cumh_sel_all, // 18	OUTPUT
                                         // common.in2_sub_cumh_sel_elem *
                                         // common.allPoints
    __global fp
        *d_in2_sub2_all, // 19	OUTPUT	common.in2_sub2_elem * common.allPoints
    __global fp
        *d_in2_sqr_all, // 20	OUTPUT	common.in2_elem * common.allPoints
    __global fp *d_in2_sqr_sub2_all, // 21	OUTPUT
                                     // common.in2_sub2_elem * common.allPoints
    __global fp
        *d_in_sqr_all, // 22	OUTPUT	common.in_elem * common.allPoints
    __global fp
        *d_tMask_all, // 23	OUTPUT	common.tMask_elem * common.allPoints
    __global fp
        *d_mask_conv_all, // 24	OUTPUT	common.mask_conv_elem * common.allPoints

    // // local
    // __local fp* d_in_mod_temp,			// 25	OUTPUT
    // common.in_elem
    // __local fp* in_partial_sum,			// 26	OUTPUT
    // common.in_cols
    // __local fp* in_sqr_partial_sum,		// 27	OUTPUT
    // common.in_sqr_rows
    // __local fp* par_max_val,				// 28	OUTPUT
    // common.mask_conv_rows
    // __local int* par_max_coo)			// 29	OUTPUT
    // common.mask_conv_rows

    // local
    __global fp *d_in_mod_temp_all,      // 25	OUTPUT	common.in_elem *
                                         // common.allPoints
    __global fp *in_partial_sum_all,     // 26	OUTPUT	common.in_cols *
                                         // common.allPoints
    __global fp *in_sqr_partial_sum_all, // 27	OUTPUT	common.in_sqr_rows *
                                         // common.allPoints
    __global fp
        *par_max_val_all, // 28	OUTPUT	common.mask_conv_rows * common.allPoints
    __global int
        *par_max_coo_all, // 29	OUTPUT	common.mask_conv_rows * common.allPoints
...

{
    ...
    // offsets for either endo or epi points (separate arrays for endo and epi
    // points)
    ...
    __global fp *d_unique_d_in2_pad_cumv =
        &d_in2_pad_cumv_all[bx * d_common.in2_pad_cumv_elem];
    __global fp *d_unique_d_in2_pad_cumv_sel =
        &d_in2_pad_cumv_sel_all[bx * d_common.in2_pad_cumv_sel_elem];
    __global fp *d_unique_d_in2_sub_cumh =
        &d_in2_sub_cumh_all[bx * d_common.in2_sub_cumh_elem];
    __global fp *d_unique_d_in2_sub_cumh_sel =
        &d_in2_sub_cumh_sel_all[bx * d_common.in2_sub_cumh_sel_elem];
    __global fp *d_unique_d_in2_sub2 =
        &d_in2_sub2_all[bx * d_common.in2_sub2_elem];
    __global fp *d_unique_d_in2_sqr =
        &d_in2_sqr_all[bx * d_common.in2_sqr_elem];
    __global fp *d_unique_d_in2_sqr_sub2 =
        &d_in2_sqr_sub2_all[bx * d_common.in2_sqr_sub2_elem];
    ...
    __global fp *d_unique_d_mask_conv =
        &d_mask_conv_all[bx * d_common.mask_conv_elem];
    ...
    __global fp *in_sqr_partial_sum =
        &in_sqr_partial_sum_all[bx * d_common.in_sqr_rows];
...
//====================================================================================================100
        ...
        if (tx == 0) {
            ...
            for (i = 0; i < d_common.no_frames; i++) {
                d_unique_d_tRowLoc[d_unique_point_no * d_common.no_frames + i] =
                    0;
                d_unique_d_tColLoc[d_unique_point_no * d_common.no_frames + i] =
                    0;
            }
        ...
        while (ei_new < d_common.in_elem) {
            ...
            d_in[col * d_common.in_rows + row] =
                d_common_change_d_frame[ori_pointer];
...
//====================================================================================================100
        ...
        in2_rowlow =
            d_unique_d_Row[d_unique_point_no] - d_common.sSize; // (1 to n+1)
        ...
        while (ei_new < d_common.in2_elem) {
            ...
            d_unique_d_in2[ei_new] =
                d_common_change_d_frame[ori_col * d_common.frame_rows +
                                        ori_row];
...
//====================================================================================================100
        ...
        while (ei_new < d_common.in_elem) {
            ...
            // d_unique_d_T[d_unique_in_pointer+rot_col*d_common.in_rows+rot_row];
            // d_in_mod_temp[ei_new] = d_unique_d_T[d_unique_point_no *
            // d_common.in_elem+rot_col*d_common.in_rows+rot_row];
            // d_in_mod_temp[ei_new] = d_unique_d_T[d_unique_point_no];
            // d_unique_d_T[d_unique_in_pointer+rot_col*d_common.in_rows+rot_row]
            // = 1;
                ...
                for (ia = ia1; ia <= ia2; ia++) {
                    ...
                    s = s +
                        d_in_mod_temp[d_common.in_rows * (ja - 1) + ia - 1] *
                            d_unique_d_in2[d_common.in2_rows * (jb - 1) + ib -
                                           1];
                }
            }
...
//==================================================50
        ...
        while (ei_new < d_common.in2_pad_cumv_elem) {
            ...
            if (row > (d_common.in2_pad_add_rows -
                ...
                d_unique_d_in2_pad_cumv[ei_new] =
                    d_unique_d_in2[ori_col * d_common.in2_rows + ori_row];
...
//==================================================50
        ...
        while (ei_new < d_common.in2_pad_cumv_cols) {
            ...
            for (position = pos_ori;
                 ...
                 position = position + 1) {
                d_unique_d_in2_pad_cumv[position] =
                    d_unique_d_in2_pad_cumv[position] + sum;
...
//==================================================50
        ...
        while (ei_new < d_common.in2_pad_cumv_sel_elem) {
            ...
            d_unique_d_in2_pad_cumv_sel[ei_new] =
                d_unique_d_in2_pad_cumv[ori_col * d_common.in2_pad_cumv_rows +
                                        ori_row];
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub_cumh_elem) {
            ...
            d_unique_d_in2_sub_cumh[ei_new] =
                d_unique_d_in2_pad_cumv[ori_col * d_common.in2_pad_cumv_rows +
                                        ori_row];
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub_cumh_rows) {
            ...
            for (position = pos_ori;
                ...
                d_unique_d_in2_sub_cumh[position] =
                    d_unique_d_in2_sub_cumh[position] + sum;
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub_cumh_sel_elem) {
            ...
            d_unique_d_in2_sub_cumh_sel[ei_new] =
                d_unique_d_in2_sub_cumh[ori_col * d_common.in2_sub_cumh_rows +
                                        ori_row];
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub2_elem) {
            ...
            // subtract
            d_unique_d_in2_sub2[ei_new] = d_unique_d_in2_sub_cumh_sel[ei_new] -
                                          d_unique_d_in2_sub2[ei_new];
...
//==================================================50
        ...
        while (ei_new < d_common.in2_pad_cumv_elem) {
            ...
            if (row > (d_common.in2_pad_add_rows -
                ...
                ori_col = col - d_common.in2_pad_add_cols;
                d_unique_d_in2_pad_cumv[ei_new] =
                    d_unique_d_in2_sqr[ori_col * d_common.in2_sqr_rows +
                                       ori_row];
            } else { // do if otherwise
                d_unique_d_in2_pad_cumv[ei_new] = 0;
            }
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub2_elem) {
            ...
            ori_row = row + d_common.in2_sub_cumh_sel2_rowlow - 1;
            ori_col = col + d_common.in2_sub_cumh_sel2_collow - 1;
            d_unique_d_in2_sqr_sub2[ei_new] =
                d_unique_d_in2_sub_cumh[ori_col * d_common.in2_sub_cumh_rows +
                                        ori_row];
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub2_elem) {
            ...
            temp2 = d_unique_d_in2_sqr_sub2[ei_new] -
...
//==================================================50
        ...
        while (ei_new < d_common.in_sqr_rows) {
            ...
            for (i = 0; i < d_common.in_sqr_cols; i++) {
                ...
                sum =
                    sum + d_unique_d_in_sqr[ei_new + d_common.in_sqr_rows * i];
            }
...
//==================================================50
        ...
        if (tx == 0) {
            ...
            for (i = 0; i < d_common.in_sqr_cols; i++) {
                ...
                in_sqr_final_sum[0] =
                    in_sqr_final_sum[0] + in_sqr_partial_sum[i];
            }
        }
...
//==================================================50
        ...
        if (tx == 0) {
            ...
            // mean = in_final_sum / d_common.in_elem;													// gets mean
            // (average) value of element in ROI
            mean =
                in_final_sum[0] /
            ...
            variance = (in_sqr_final_sum[0] / d_common.in_elem) -
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub2_elem) {
            ...
            // d_unique_d_in2_sqr_sub2[ei_new] = d_unique_d_in2_sqr_sub2[ei_new]
            // * denomT;
            d_unique_d_in2_sqr_sub2[ei_new] =
                d_unique_d_in2_sqr_sub2[ei_new] * denomT[0];
...
//==================================================50
        ...
        while (ei_new < d_common.conv_elem) {
            ...
            // d_unique_d_conv[ei_new] = d_unique_d_conv[ei_new] -
            // d_unique_d_in2_sub2[ei_new] * in_final_sum / d_common.in_elem;
            d_unique_d_conv[ei_new] = d_unique_d_conv[ei_new] -
                                      d_unique_d_in2_sub2[ei_new] *
                                          in_final_sum[0] / d_common.in_elem;
...
//==================================================50
        ...
        while (ei_new < d_common.in2_sub2_elem) {
            ...
            d_unique_d_in2_sqr_sub2[ei_new] =
                d_unique_d_conv[ei_new] / d_unique_d_in2_sqr_sub2[ei_new];
...
//==================================================50
        ...
        if (d_frame_no == 0) {
            tMask_row = cent + d_unique_d_Row[d_unique_point_no] -
                        d_unique_d_Row[d_unique_point_no] - 1;
            tMask_col = cent + d_unique_d_Col[d_unique_point_no] -
                        d_unique_d_Col[d_unique_point_no] - 1;
        } else {
            pointer = d_unique_point_no * d_common.no_frames + d_frame_no - 1;
            tMask_row = cent + d_unique_d_tRowLoc[pointer] -
                        d_unique_d_Row[d_unique_point_no] - 1;
            tMask_col = cent + d_unique_d_tColLoc[pointer] -
                        d_unique_d_Col[d_unique_point_no] - 1;
        }
        ...
        while (ei_new < d_common.mask_conv_elem) {
            ...
            for (ja = ja1; ja <= ja2; ja++) {
                ...
                for (ia = ia1; ia <= ia2; ia++) {
                    ib = ip1 - ia;
                    s = s +
                        d_unique_d_tMask[d_common.tMask_rows * (ja - 1) + ia -
                                         1] *
                            1;
                }
            }
        ...
        while (ei_new < d_common.mask_conv_rows) {
            ...
            for (i = 0; i < d_common.mask_conv_cols; i++) {
                ...
                largest_value_current =
                    fabs(d_unique_d_mask_conv[largest_coordinate_current]);
...
//==================================================50
        ...
        if (tx == 0) {
            ...
            offset_col = largest_col - d_common.in_cols -
                         (d_common.sSize - d_common.tSize);
            pointer = d_unique_point_no * d_common.no_frames + d_frame_no;
            d_unique_d_tRowLoc[pointer] =
                d_unique_d_Row[d_unique_point_no] + offset_row;
            d_unique_d_tColLoc[pointer] =
                d_unique_d_Col[d_unique_point_no] + offset_col;
        }
...
#ifdef TEST_CHECKSUM
        if (bx == 0 && tx == 0) {
            checksum[35] = checksum[35] + d_unique_d_tRowLoc[pointer] +
                           d_unique_d_tColLoc[pointer];
        }
...
//====================================================================================================100
        ...
        while (ei_new < d_common.in_elem) {
            ...
            d_in[ei_new] =
                ...
                (1 - d_common.alpha) * d_common_change_d_frame[ori_pointer];
