...
void kernel_gpu_opencl_wrapper(record *records, long records_mem, knode *knodes,
    ...
    // Create kernel
    cl_kernel kernel;
    kernel = clCreateKernel(program, "findK", &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_mem recordsD;
    recordsD =
        clCreateBuffer(context, CL_MEM_READ_WRITE, records_mem, NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_mem keysD;
    keysD = clCreateBuffer(context, CL_MEM_READ_WRITE, count * sizeof(long),
                           NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    cl_mem ansD;
    ansD = clCreateBuffer(context, CL_MEM_READ_WRITE, count * sizeof(record),
                          NULL, &error);
    if (error != CL_SUCCESS)
        fatal_CL(error, __LINE__);
    ...
    clSetKernelArg(kernel, 3, sizeof(cl_mem), (void *)&recordsD);

    clSetKernelArg(kernel, 4, sizeof(cl_mem), (void *)&currKnodeD);
    clSetKernelArg(kernel, 5, sizeof(cl_mem), (void *)&offsetD);
    clSetKernelArg(kernel, 6, sizeof(cl_mem), (void *)&keysD);
    clSetKernelArg(kernel, 7, sizeof(cl_mem), (void *)&ansD);
