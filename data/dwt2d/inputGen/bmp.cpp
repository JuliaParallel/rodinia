...
void Bmp::setPixel(int x, int y, unsigned char red, unsigned char green,
                   unsigned char blue) {
    bool doSet = true;

    if (x < 0 || x >= mWidth) {
        cout << "Invalid value: " << x << " (expected: 0 <= x < " << mWidth
             << ")" << endl;
    ...
    if (y < 0 || y >= mHeight) {
        cout << "Invalid value: " << y << " (expected: 0 <= y < " << mHeight
             << ")" << endl;
...
void Bmp::setPixel(int x, int y, const Color &theColor) {
...
void Bmp::drawRectangle(int x, int y, int width, int height, unsigned char red,
...
void Bmp::drawRectangle(int x, int y, int width, int height,
                        const Color &theColor) {
    drawRectangle(x, y, width, height, theColor.mRed, theColor.mGreen,
                  theColor.mBlue);
}
...
void Bmp::fillRectangle(int x, int y, int width, int height, unsigned char red,
                        unsigned char green, unsigned char blue) {
    for (int i = 0; i < width; i++) {
        drawLine(x + i, y, x + i, y + height, red, green, blue);
    }
}
...
void Bmp::fillRectangle(int x, int y, int width, int height,
                        const Color &theColor) {
    fillRectangle(x, y, width, height, theColor.mRed, theColor.mGreen,
                  theColor.mBlue);
}
...
void Bmp::fillPolygon(const vector<int> &points, unsigned char red,
                      unsigned char green, unsigned char blue) {
    int xMin = 0;
    int yMin = 0;
    int xMax = 0;
    ...
    for (int i = 0, n = theBuffer.size(); i < n; i++) {
        for (int j = 1, nn = theBuffer[i].size(); j < nn; j += 2) {
            drawLine(xMin + i, theBuffer[i][j], xMin + i, theBuffer[i][j - 1],
                     red, green, blue);
        }
    }
}
...
void Bmp::drawLine(int x0, int y0, int x1, int y1, unsigned char red,
                   unsigned char green, unsigned char blue) {
    int xDelta = (x1 - x0);
    int yDelta = (y1 - y0);
...
void Bmp::drawLine(int x0, int y0, int x1, int y1, const Color &theColor) {
    drawLine(x0, y0, x1, y1, theColor.mRed, theColor.mGreen, theColor.mBlue);
}
...
void Bmp::drawPolyline(const vector<int> &points, unsigned char red,
                       unsigned char green, unsigned char blue) {
    int xPrevious = 0;
    int yPrevious = 0;
...
int Bmp::getWidth() const { return (mWidth); }

// ----------------------------------------------------------------
// The bitmap height
// ----------------------------------------------------------------

int Bmp::getHeight() const { return (mHeight); }
...
bool Bmp::write(string &fileName, string &errMsg) const {
    ...
    int fileSize =
        mWidth * mHeight * 3 + BMP_FILE_HEADER_SIZE + BMP_INFO_HEADER_SIZE;
