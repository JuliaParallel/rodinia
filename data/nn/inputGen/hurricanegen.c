 ...
 * (nn)
 * Also generates lists of the files in the dataset. These lists are passed to
 * nn.
 *
...
int main(int argc, char **argv) {
    ...
    canes = (total_canes + num_files - 1) /
            num_files; // round up (ceiling division)
    ...
    for (j = 0; j < num_files; j++) {
        if (num_files == 1)
            ...
            sprintf(fname, "cane%dk_%d_%d.db", total_canes / 1024, num_files,
                    j);
