...
void ellipsetrack(avi_t *video, double *xc0, double *yc0, int Nc, int R, int Np,
    ...
    */
    ...
    for (frame_num = 1; frame_num <= Nf; frame_num++) {
        ...
        // Initialize the current positions to be equal to the previous
        // positions
        for (i = 0; i < Nc; i++) {
        ...
        for (cell_num = 0; cell_num < Nc; cell_num++) {
            ...
            ycavg[cell_num] = 0.0;
            for (i = (frame_num > 10 ? frame_num - 10 : 0); i < frame_num;
                 i++) {
            ...
            ycavg[cell_num] =
                ycavg[cell_num] / (double)(frame_num > 10 ? 10 : frame_num);
            ...
            Isub[cell_num] = m_get(v2[cell_num] - v1[cell_num] + 1,
                                   u2[cell_num] - u1[cell_num] + 1);
            for (i = v1[cell_num]; i <= v2[cell_num]; i++) {
                for (j = u1[cell_num]; j <= u2[cell_num]; j++) {
                    m_set_val(Isub[cell_num], i - v1[cell_num],
                              j - u1[cell_num], m_get_val(I, i, j));
                }
            }
        ...
        for (cell_num = 0; cell_num < Nc; cell_num++) {
            ...
            ellipseevolve(IMGVF[cell_num], &(xci[cell_num]), &(yci[cell_num]),
                          ri[cell_num], t, Np, (double)R, ycavg[cell_num]);
            ...
            for (j = 0; j < Np; j++) {
                ...
                x[cell_num][j][frame_num] =
                    xc[cell_num][frame_num] + (ri[cell_num][j] * cos(t[j]));
                y[cell_num][j][frame_num] =
                    yc[cell_num][frame_num] + (ri[cell_num][j] * sin(t[j]));
            }
        ...
        if (getenv("OUTPUT") && frame_num == Nf) {
            FILE *pFile;
            pFile = fopen("result.txt", "w+");

            for (cell_num = 0; cell_num < Nc; cell_num++)
                fprintf(pFile, "\n%d,%f,%f", cell_num, xci[cell_num],
                        yci[cell_num]);
