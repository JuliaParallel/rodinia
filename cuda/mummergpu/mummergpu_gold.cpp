...
#pragma omp parallel for
        ...
        for (int i = 0; i < numQueries; ++i) {
            rc_kernel_gold(i, results, refstr, queries, queryAddrs,
                           queryLengths, nodeTexture, childrenTexture,
                           numQueries, match_length);
        }
...
#pragma omp parallel for
        ...
        for (int i = 0; i < numQueries; ++i) {
            ...
            #endif
            */
            kernel_gold(i, results, queries, nodeTexture, childrenTexture,
                        refstr, queryAddrs, queryLengths, numQueries,
                        match_length
#if TREE_ACCESS_HISTOGRAM
                        ,
                        int *node_hist, int *child_hist
