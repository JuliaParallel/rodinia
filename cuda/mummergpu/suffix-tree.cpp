  ...
  public:
    ...
    void printNodeDot(SuffixNode *node, ostream &dfile) {
        ...
        if (children == 0) {
            ...
            // dfile << " " << node << " [label=";
            // node->printLabel(dfile, m_string) << "]" << endl;
            dfile << " " << node << " [width=.2,height=.2,label=\""
                  << node->id() << "\"]" << endl;
        }
    }
    ...
    void printTreeFlat(ostream &out) {
        out << "nodeid\tparent\tSL\tstart\tend\t$\tA\tC\tG\tT\tnodestring"
            << endl;
    ...
    int verifyNodeSuffixLinks(SuffixNode *node, int &linkcount) {
        ...
        if (node != m_root && node->m_suffix) {
            ...
            if (np.substr(1, np.length() - 1) != sp) {
                ...
                node->m_suffix->printLabel(cerr, m_string) << ": " << sp
                                                           << endl;
                errs++;
            }
        ...
        if (childcount && !node->m_suffix) {
            errs++;
            node->printLabel(cerr, m_string) << " has no suffix link!!!"
                                             << endl;
        }
...
// Construct T1
        ...
        for (int i = 2; i <= len; i++) {
            ...
            // Keep track of last internal nodes created in split so we can add
            // suffix links
            ...
            for (int j = startj; j <= i; j++) {
                ...
                // Precondition: S[j-1 .. i] (alpha) is in the suffix tree
                // "near" node
                //               All Internal nodes have a suffix link

                // Idea: 1) Remember where alpha is in the tree relative to node
                //       2) Walk up the tree w bases until we get to a node with
                //       a suffix link.
                //       3) Follow suffix link which shifts the path from
                //       S[j-1..i] to S[j..i]
                //       4) Walk down tree in new location ensuring S[i-w .. i]
                //       is in tree

                // Notes: 1) All internal nodes have a suffix link by next
                // extension
                //        2) Any time we walk up to root, have to check S[j..i]
                //        3) Suffix [1..i] is always present so start extension
                //        j with 2
                ...
                if (DEBUG) {
                    ...
                    cerr << i << "." << j << " Phase " << i << " Extension "
                         << j << ": \"" << beta << "\" bp:" << betapos << endl;

                    cerr << i << "." << j << "  Walking up from n:";
                    node->printLabel(cerr, m_string) << " nw: " << nodewalk
                                                     << endl;
                }

                if (node == m_root) {
                    // If we are at root, we have to check the full string
                    // s[j..i] anyways
                } else {
                    if (nodewalk) {
                        // partially walked down node->child, but didn't switch
                        // to child
                        // Match at i=6 on left... nodewalk=2, at 5 after suffix
                        // link
                        // 5 = i-2+1
                        //                 o ----- o
                        //               5 A       A 5  <-
                        ...
                        if (DEBUG) {
                            cerr << i << "." << j << "   Adjusted len: " << len
                                 << endl;
                        }
                    }

                    if (DEBUG) {
                        cerr << i << "." << j << "   parent bp: " << betapos
                             << " n:";
                        node->printLabel(cerr, m_string) << endl;
                    }
                ...
                if (DEBUG) {
                    cerr << i << "." << j
                         << "  Starting to walk down from bp: " << betapos
                         << " to " << i << " n:";
                    node->printLabel(cerr, m_string) << endl;
                }
                ...
                if (FORCEROOT && node != m_root) {
                    ...
                    if (DEBUG) {
                        cerr << i << "." << j << " AtRoot bp: " << betapos
                             << endl;
                    }
                }

                bool done = false;
                startj = j + 1; // assume this extension should be skipped in
                                // the next phase
                ...
                while ((betapos <= i) && !done) {
                    ...
                    if (!child) {
                        if (splitnode && betapos == splitnode->m_start) {
                            if (DEBUG) {
                                cerr << i << "." << j << "   Add SL1: ";
                                splitnode->m_parent->printLabel(cerr, m_string)
                                    << " sl-> ";
                                node->printLabel(cerr, m_string) << endl;
                            }
                        ...
                        SuffixNode *newnode = new SuffixNode(
                            betapos, len, j, node, NULL); // leaf: j
#endif
                        ...
                        // This is the first base that differs, but the
                        // edgelength to
                        // i may be longer. Therefore set nodewalk to 0, so the
                        // entire
                        // edge is subtracted.
                        nodewalk = 0;
                        done = true;
                        ...
                        if (nodebase != base) {
                            ...
                            cerr << "ERROR: first base on edge doesn't match "
                                    "edge label"
                                 << endl;
                            cerr << "       nb: " << nb << " base: " << base
                                 << endl;
                            exit(1);
                        }
#endif

                        // By construction, the string from j-1 to betapos to
                        // i-1
                        // must already by present in the suffix tree
                        // Therefore, we can skip checking every character, and
                        // zoom
                        // to exactly the right character, possibly skipping the
                        // entire edge
                        ...
                        if (DOJUMP) {
                            ...
                            if (mustmatch >= childlen) {
                                ...
                                if (nodepos != child->m_end + 1) {
                                    cerr << "ERROR: jump should have skipped "
                                            "entire edge, but didn't!"
                                         << endl;
                                    exit(1);
                                }
#endif
...
#if VERIFY
                                if (VERIFY) {
                                    if (m_string[betapos - 1] !=
                                        m_string[nodepos - 1]) {
                                        cerr << "ERROR: jump should have "
                                                "matched at least the "
                                                "mustmatch-1 characters"
                                             << endl;
                                        cerr << "s[bp-1]: "
                                             << m_string[betapos - 1]
                                             << " s[np-1]: "
                                             << m_string[nodepos - 1] << endl;
                                        exit(1);
                                    }
                                }
#endif
                        ...
                        while (nodepos <= child->m_end && betapos <= i) {
                            ...
                            if (m_string[betapos] == nodebase) {
                                if (splitnode &&
                                    betapos == splitnode->m_start) {
                                    if (DEBUG) {
                                        cerr << i << "." << j << "   Add SL2: ";
                                        splitnode->m_parent->printLabel(
                                            cerr, m_string)
                                            << " sl-> ";
                                        node->printLabel(cerr, m_string)
                                            << endl;
                                ...
                                if (betapos == i + 1) {
                                    if (DEBUG) {
                                        cerr << i << "." << j
                                             << "    Internal edge match nw: "
                                             << nodewalk << endl;
                                    }

                                    if ((nodewalk == child->len(i)) &&
                                        (child->m_end == len)) {
                                        // we walked the whole edge to leaf,
                                        // implicit rule I extension
                                        if (DEBUG) {
                                            cerr << i << "." << j
                                                 << "    Leaf Node, Implicit "
                                                    "Rule I Extension"
                                        ...
                                        // The j-1 extension was the last
                                        // explicit extension in this round
                                        // Start the next round at the last
                                        // explicit extension
                                        if (DOPHASETRICK) {
                                            startj = j;
                                        ...
                                        if (DOINTERNALSKIP) {
                                            // Since we hit an internal match on
                                            // a non-leaf, we know every other
                                            // extension in this phase will also
                                            // hit an internal match.

                                            // Have to be careful since leafs
                                            // get the full string immediately,
                                            // but
                                            // they really have a Rule 1
                                            // extension
...
#ifdef MPOOL
                                SuffixNode *split = new (&m_pool)
                                    SuffixNode(child->m_start, nodepos - 1, 0,
                                               node, NULL); // internal
#else
                                SuffixNode *split =
                                    new SuffixNode(child->m_start, nodepos - 1,
...
// Now create the new node
#ifdef MPOOL
                                SuffixNode *newnode = new (&m_pool) SuffixNode(
                                    betapos, len, j, split, NULL); // leaf j
#else
                                SuffixNode *newnode = new SuffixNode(
                                    betapos, len, j, split, NULL); // leaf j
#endif
                                ...
                                split->m_children[b2i(m_string[betapos])] =
                                    newnode;
                                splitnode = newnode;
                                ...
                                // This is the first base that differs, but the
                                // edgelength to
                                // i may be longer. Therefore set nodewalk to 0,
                                // so the entire
                                // edge is subtracted.
                                nodewalk = 0;
                                done = true;
...
void buildSuffixTreeTexture(PixelOfNode **nodeTexture,
    ...
    *childrenTexture = NULL;
    ...
    if (*children_height) {
        *childrenTexture = (PixelOfChildren *)calloc(
    ...
    *aux_data =
        (AuxiliaryNodeData *)calloc(allnodes, sizeof(AuxiliaryNodeData));

    if (!*nodeTexture || (*children_height && !*childrenTexture) ||
        !*aux_data) {
                            ...
                            for (int k = 0; k < basecount; k++) {
                                ...
                                if (ggchild) {
                                    ...
                                    for (int l = 0; l < basecount; l++) {
                                        SuffixNode *gggchild =
                                            ggchild->m_children[l];

                                        if (gggchild) {
                                            gggchild->m_nodeid = ++nodecount;

                                            for (int m = 0; m < basecount;
                                                 m++) {
                                                SuffixNode *ggggchild =
                                                    gggchild->m_children[m];
                                                if (ggggchild) {
                                                    nodequeue.push(make_pair(
                                                        ggggchild, depth + 5));
                                                }
                                            }
                                        }
...
getQueriesTexture(int qfile, char **queryTexture, size_t *queryTextureSize,
        ...
        for (; i < bytes_read; i++) {
            ...
            if (b == '>') {
                ...
                if (offsetspos != 0) {
                    if (this_qrylen < min_match_length) {
                        ...
                        if (warp_max_qry_len < this_qrylen + 2) {
                            ...
                            curr_warp_padding -=
                                num_warp_queries * warp_max_qry_len;
                            curr_mem_usage -=
                                num_warp_queries * warp_max_qry_len;
                        ...
                        // we really ought to pass the real length in, and then
                        // add the padding
                        // For now, we are overestimating.
                        curr_mem_usage += bytesNeededOnGPU(warp_max_qry_len,
                                                           min_match_length);
                        curr_warp_padding += warp_max_qry_len;
#else
                        curr_mem_usage +=
                            bytesNeededOnGPU(this_qrylen, min_match_length);
#endif
                    }
                }
                ...
                if (offsetspos == offsetssize) {
                    offsetssize *= 2;
                    offsets =
                        (int *)realloc(offsets, sizeof(int) * offsetssize);
                    lengths =
                        (int *)realloc(lengths, sizeof(int) * offsetssize);
                    if (!offsets || !lengths) {
                        cerr << endl
    ...
    if (qstringpos) {
        if (this_qrylen < min_match_length) {
            ...
            if ((++num_warp_queries) % WARP_SIZE) {
                curr_warp_padding +=
                    warp_max_qry_len *
                    (WARP_SIZE - (num_warp_queries % WARP_SIZE));
            }
#else
    ...
    for (unsigned int i = 0; i < *numQueries; i += WARP_SIZE) {
        ...
        for (int curr_char = 0; curr_char < max_qry_len; ++curr_char) {
            for (unsigned int curr_qry = 0;
                 curr_qry < WARP_SIZE && (i + curr_qry) < *numQueries;
                 ++curr_qry) {
                ...
                if (curr_char < qlen) {
                    int *qry_word = reordered_query_chars +
                                    ((curr_char / 4) * WARP_SIZE + curr_qry);
                    int c = *(qstring + offsets[i + curr_qry] + curr_char);
                    //*((char*) ((void*)qry_word + (curr_char & 0x0000000f) = c;
                    //*qry_word |= c << ((curr_char & 0x0000000f) << (3 -
                    //(curr_char & 0x0000000f)));
    ...
    *num_match_coords = 0;
    for (unsigned int i = 0; i < reordered_query_chunks.size(); ++i) {
        for (unsigned int j = 0;
             j < WARP_SIZE && (i * WARP_SIZE + j) < *numQueries; ++j) {
    ...
    *num_match_coords =
        *queryTextureSize - *numQueries * (min_match_length + 1);
#endif
    assert(curr_mem_usage < memory_avail);
    cerr << offsetspos << " queries (" << qrylen << " bp), need "
         << curr_mem_usage << " bytes on the GPU (" << memory_avail
         << " avail) " << timer.str(true, 5) << endl;
}
...
void printNodeAlignments(const ReferencePage *page,
    ...
    if (isLeaf) {
        ...
        if (child.data) {
            printNodeAlignments(page, queryflankingbase, child, qrypos, qrylen,
                                path, path_idx, rc);
        }
...
void printAlignments(ReferencePage *page, Alignment *h_alignments, char *query,
    ...
    while (MK3(SDEPTH(cur)) > min_match) {
        ...
        if (verbose) {
            cout << "par: " << PADDR(cur) << " depth: " << MK3(SDEPTH(cur))
                 << endl;
        }
    }
                ...
                if (depthToGoldenPath == 0) {
                    if (curchild == goldenChild) {
                        ...
                        if (cid == matchNodeId) {
                            ...
                            if (edge_match > 0) {
                                matchlen = MK3(SDEPTH(cur)) -
                                           (NODE_LENGTH(cid) - edge_match) - 1;
                            }
                        }
                    } else {
                        depthToGoldenPath = 1;
