...
void printHelp() {
    fprintf(
        ...
        "  -C             Compute the matches using the CPU instead of the "
        "GPU\n"
...
void ParseCommandLine(int argc, char **argv) {
    ...
    while (!errflg &&
           ((ch = getopt(argc, argv, "aCchql:d:t:s:brcLM")) != EOF)) {
        switch (ch) {
        case 'h':
            printHelp();
            break;
...
int main(int argc, char *argv[]) {
    ...
    MatchContext ctx;
    if ((err = createMatchContext(
             &ref, &queries, 0, OPT_on_cpu, OPT_match_length, OPT_stats_file,
             OPT_reverse, OPT_forwardreverse, OPT_forwardcoordinates,
             OPT_showQueryLength, OPT_dotfilename, OPT_texfilename, &ctx))) {
        printStringForError(err);
        exit(err);
    }
