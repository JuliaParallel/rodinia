...
struct Reference {
    ...
    void *d_node_tex_array; // really a cudaArray* (or PixelOfNode* if NODETEX
                            // == 1)
    void *h_node_tex_array; // really a PixelOfNode*

    void *d_children_tex_array; // cudaArray* (or PixelOfChildren* if CHILDTEX
                                // == 1)
...
struct MatchCoord {
    union {
        ...
        struct {
            ...
            int edge_match_length; // number of missing characters UP the parent
                                   // edge
        };
    };
};
...
int createMatchContext(Reference *ref, QuerySet *queries, MatchResults *matches,
                       bool on_cpu, int min_match_length, char *stats_file,
                       bool reverse, bool forwardreverse,
                       bool forwardcoordinates, bool showQueryLength,
                       char *dotfilename, char *texFilename, MatchContext *ctx);
