...
#define MATCH_BASE(match_coords, qryid)                                        \
    (_MatchCoord *) match_coords + coordAddrs[qryid]
#else
#define RESULT_SPAN 1
#define MATCH_BASE(match_coords, qryid)                                        \
    (_MatchCoord *) match_coords + qryAddr - __umul24(qryid, min_match_len + 1)
#endif
...
#define GETNODEHIST(addr, two_level)                                           \
    getMerged(nodes, childrenarr, addr, two_level, 0, node_hist, child_hist)
#define GETCHILDREN(addr, two_level)                                           \
    getMerged(nodes, childrenarr, addr, two_level, 1, NULL, NULL)
#define GETCHILDRENHIST(addr, two_level)                                       \
    getMerged(nodes, childrenarr, addr, two_level, 1, node_hist, child_hist)
#endif
...
#define GETNODEHIST(addr, two_level)                                           \
    getMerged(nodes, childrenarr, addr, two_level, 0)
#define GETCHILDREN(addr, two_level)                                           \
    getMerged(nodes, childrenarr, addr, two_level, 1)
#define GETCHILDRENHIST(addr, two_level)                                       \
    getMerged(nodes, childrenarr, addr, two_level, 1)
#endif
#endif
...
#if TREE_ACCESS_HISTOGRAM
#define GETCHILDRENHIST(addr, two_level)                                       \
    getChildren(addr, two_level, childrenarr, child_hist)
#define GETCHILDREN(addr, two_level)                                           \
    getChildren(addr, two_level, childrenarr, NULL)
#else
#define GETCHILDRENHIST(addr, two_level)                                       \
    getChildren(addr, two_level, childrenarr)
...
struct __align__(8) _MatchCoord {
    union {
        ...
        struct {
            int node;              // match node
            int edge_match_length; // number of matching characters UP the
                                   // parent edge
        };
    };
};
...
#if REORDER_TREE
    addr = (arr.x | ((arr.z & 0xF) << 8)) |
           ((arr.y | ((arr.z & 0xF0) << 4)) << 16);
#else
    addr = MKI(arr);
#endif
}
...
__device__ void printNode(int nodeid
    ...
    XPRINTF("%d\t" fNID "\t%d\t%d\t%d\t%d\t" fNID "\t" fNID "\t" fNID "\t" fNID
            "\t" fNID "\t" fNID "\t" fNID "\n",
            nodeid, NID(addr), start, end, depth, leafchar, NID(a), NID(c),
            NID(g), NID(t), NID(d), NID(p), NID(s));
}
...
#else
#define SET_RESULT(c, r, e, q, m, rc)                                          \
    set_result(c, r, e, q, m, rc, nodes, childrenarr)
#endif
...
__device__ void set_result(unsigned int cur, _MatchCoord *result,
                           int edge_match_length, int qry_match_len,
    ...
    if (qry_match_len > min_match_len) {
        ...
        XPRINTF("  saving match cur=%d " fNID
                " len=%d edge_match=%d depth=%d\n",
                result->data.x, NID(cur), qry_match_len, edge_match_length,
                MKI(nd.depth));
...
__global__ void mummergpuKernel(void *match_coords,
    ...
    for (int qrystart = 0; qrystart <= last;
         qrystart++, result += RESULT_SPAN) {
        //_PixelOfNode node;
...
__global__ void mummergpuRCKernel(MatchCoord *match_coords, char *queries,
                                  const int *queryAddrs,
                                  const int *queryLengths, const int numQueries,
    ...
    /*
       ...
       int qryAddr=queryAddrs[qryid];
       MatchCoord * result = match_coords + qryAddr - __umul24(qryid,
    min_match_len + 1);
       queries += qryAddr;
    ...
    qry_match_len);
             ...
             if (cur.data == 0)
                    ...
                    SET_RESULT(prev, (_MatchCoord*)result, 0, qry_match_len,
    min_match_len,
             ...
             {
                       node.data = tex2D(nodetex, cur.data & 0xFFFF, cur.data >>
    16);
             }
    ...
    #if !REFTEX
                                                            //FIXME: this needs
    to be a pointer to ref->d_ref_array
                                                            ,NULL
    #endif
    );
    ...
    qry_match_len,refpos - (MKI(node.start)));
          ...
          SET_RESULT(cur, (_MatchCoord*)result, refpos - MKI(node.start),
    qry_match_len,
...
#endif
                            const int *queryAddrs, const int *queryLengths,
                            const int page_begin, const int page_end,
        ...
        if (isLeaf || forceToParent) {
            ...
            if (isLeaf && isLeaf != queryflankingbase) {
                ...
                if ((left_in_ref != page_begin || page_shadow_left == -1) &&
                    ...
                    if (!(left_in_ref > page_begin &&
                          right_in_ref < page_shadow_left)) {
                        // sprintf(buf, "%8d%10d%10d\n", left_in_ref,
                        // qrystartpos+1, matchlen);
                        XPRINTF("%8d%10d%10d\n", left_in_ref,
                                matches[matchid].qrystartpos + 1, matchlen);
            ...
            if (depthToGoldenPath == 0) {
                ...
                goldenChild =
                    GETQCHAR(matches[matchid].qrystartpos + matchlen + 1);
            }
