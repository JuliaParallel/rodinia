 ...
 * the terms of the
 * MIT License. Read the full licence:
...
/* HUFFMAN-FRIENDLY PAVLE
   ...
   SYMBOLS (BYTES).
   -	NUMBER OF THREADS PER BLOCK IS 256; IF YOU WANT TO PLAY WITH DIFFERENT
   NUMBERS, THE CW CACHING SHOULD BE MODIFIED (SEE DPT* KERNELS)
...
vlc_encode_kernel_sm64huff(unsigned int *data, const unsigned int *gm_codewords,
    ...
    // number of branches
    // else
    atomicOr(&as[kc], tmpcw32 << (32 - startbit - wrbits)); // shift left in
                                                            // case it's shorter
                                                            // then the
                                                            // available space
    codewordlen -= wrbits;
