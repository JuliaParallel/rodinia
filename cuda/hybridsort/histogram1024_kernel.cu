...
__device__ void addData1024(volatile unsigned int *s_WarpHist,
...
__global__ void histogram1024Kernel(unsigned int *d_Result, float *d_Data,
                                    float minimum, float maximum, int dataN) {
    ...
    // Declare as volatile to prevent incorrect compiler optimizations in
    // addPixel()
    ...
    for (int pos = globalTid; pos < dataN; pos += numThreads) {
        unsigned int data4 =
            ((d_Data[pos] - minimum) / (maximum - minimum)) * BIN_COUNT;
...
void histogram1024GPU(unsigned int *h_Result, float *d_Data, float minimum,
                      float maximum, int dataN) {
    checkCudaErrors(cudaMemset(d_Result1024, 0, HISTOGRAM_SIZE));
    histogram1024Kernel<<<BLOCK_N, THREAD_N>>>(d_Result1024, d_Data, minimum,
                                               maximum, dataN);
    checkCudaErrors(cudaMemcpy(h_Result, d_Result1024, HISTOGRAM_SIZE,
